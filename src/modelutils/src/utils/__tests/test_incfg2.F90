...
!!$      result_S(0,1:7) = (/'la','al','mg','vg','sd','tm','vf'/)
   ...
   subroutine test_incfg_chk(F_filename_S,result_S,F_nvar,F_nstep,F_msg_S)
      ...
      integer :: istat,istep,index,dt,ii,nbvar,ivar,lvl0,lvl1,ip1list(99),ip1list01(99),ip1list02(99),nip1,ipkind
      real :: zp1
      type(INCFG_T) :: incfgobj
      ...
      nbvar = incfg_nbvar(incfgobj)
      call testutils_assert_eq(nbvar,F_nvar,'nbvar')
      write(string_S,*) 'incfg file opened: ',trim(F_filename_S),nbvar,F_nstep
      call msg(MSG_DEBUG,string_S)
      STEPLOOP: do istep = 0,F_nstep
         ii = 0
         VARLOOP: do ivar=1,nbvar
            istat = incfg_isvarstep(incfgobj,ivar,istep)
            if (.not.RMN_IS_OK(istat)) cycle VARLOOP
            ip1list = -1
            istat = incfg_meta(incfgobj,ivar,varname_S,varname2_S,F_lvl0=lvl0,F_lvl1=lvl1,F_ip1list=ip1list,F_nip1=nip1)
            if (.not.RMN_IS_OK(istat)) cycle VARLOOP
