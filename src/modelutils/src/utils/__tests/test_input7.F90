...
#include <rmnlib_basics.hf>
   ...
   write(dir_S,'(a,i4.4)') '__to-delete_test_input7-',myproc
...
!!$   inrep_S = trim(dfiles_S)//'/bcmk'
   ...
   filename_S='../test_input7_table'

   istat = clib_getenv('TEST_SUBNAME', name_S)
   if (name_S == 'odd') then
      call testutils_set_name('test_input7_odd')
      call test_input7_get_data_odd(trim(filename_S))
   else
      call testutils_set_name('test_input7_even')
      call test_input7_get_data(trim(filename_S))
   endif
...
contains
   ...
   !/@
   subroutine test_input7_get_data(F_filename_S)
      implicit none
      ...
      integer :: istat,inputid,dateo,istep,dt,ii,gridid,nbvar,ivar,nhyb,k,ikind,ip1list(NK),lclgridid
      ...
      integer :: ijkmin(3),ijkmax(3),ilvl,vals2(10),allvals2(10,NSTEP+1,NVAR,26)
...
!!$      skip_list_S(1:3) = (/'xa  ','icel','fsa '/)
      ...
      ok_L = .false.
      gridid = ezqkdef(GNI,GNJ, 'G', 0,0,0,0,0) !#TODO: define a real LAM (Z/L)
      ...
      lijk0(1:3) = (/mini, minj, 1/)
      uijk0(1:3) = (/maxi, maxj, NK/)

      call gmm_build_meta2D(p0meta, &
           mini,maxi,HALO,HALO,lni, &
           minj,maxj,HALO,HALO,lnj, &
           0,GMM_NULL_FLAGS)
      nullify(p0data)
...
!!$      inputid = input_new(jdate_to_cmc(jdateo), dt, F_filename_S, ip1list(1:nhyb))
      inputid = input_new(jdateo, dt, F_filename_S)
      if (.not.RMN_IS_OK(inputid)) then
         call msg(MSG_ERROR,'test_input7 - cannot init input_mod for: '//trim(F_filename_S))
         return
      endif

      nbvar = input_nbvar(inputid)
      call testutils_assert_eq(nbvar, NVAR+NSKIP, 'nbvar')

      istat = input_set_basedir(inputid, '.')
...
!!$            print *,'============================================',istep,ivar
            ...
            if (.not.(RMN_IS_OK(istat) .and. associated(data))) then
               call msg(MSG_WARNING,'test_input7 - var not found: '//trim(varname_S))
...
!!$            print *,'====',inputobj%fid%files(1:inputobj%fid%nfiles)%unit
            ...
            lijk1 = lbound(data)
            uijk1 = ubound(data)
            write(dummy_S, '(a,2i3,a,2i3,a,2i3,a,2i3,a)') "exp(",lijk0(1:2),":",uijk0(1:2),") => got(",lijk1(1:2),":",uijk1(1:2),")"
            call testutils_assert_ok( &
                 all(lijk1(1:2) == lijk0(1:2)) .and. &
                 all(uijk1(1:2) == uijk0(1:2)), 'bounds '//trim(varname_S)//':'//trim(step_S)//":"//trim(dummy_S))
...
!!$                  print *,'fact: '//varname_S(1:2),fact
      ...
      enddo STEPLOOP
      istat = input_close_files(inputid)
      ! ---------------------------------------------------------------------
      return
   end subroutine test_input7_get_data


   !/@
   subroutine test_input7_get_data_odd(F_filename_S)
      implicit none
      character(len=*),intent(in) :: F_filename_S
      !@/
      integer,parameter :: NSTEP = 1
...
!!$                  print *,'fact: '//varname_S(1:2),fact
      ...
      ! ---------------------------------------------------------------------
      return
   end subroutine test_input7_get_data_odd

end subroutine test_input7
