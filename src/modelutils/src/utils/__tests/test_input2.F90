...
#include <clib_interface_mu.hf>
   ...
   write(dir_S,'(a,i4.4)') '__to-delete_test_input2-',myproc
...
contains
   ...
   subroutine test_input2_get_data(F_filename_S)
      ...
      integer :: istat,inputid,dateo,istep,index,dt,ii,gridid,nbvar,ivar,nhyb,k,ikind
      logical :: ok_L(0:NSTEP,NVAR)
      real, pointer :: data(:,:,:),data2(:,:,:)
      integer :: vals(8),allvals(8,NSTEP+1,NVAR),ip1list(NK),nn
...
!!$      result_S(1,1:2) = (/'tm','hs'/)
      result_S(0,1:NVAR) = (/ &
           ...
           'i2  ','i3  ','i4  ','i6  ','j1  ', &
!!$           'j2  ','dn  ','icel','al  '/)
!!$      !TODO: add up-air fields and vect fields
           'j2  ','dn  ','icel','al  ','tt  ', &
           'uu  ','vv  '/)
...
!!$      allvals(:,1,13) = (/      23340,      30109, 0, 0, 0, 0, 0, 0/) !i0
      ...
      allvals(:,1,18) = (/       -294,      92370, 0, 0, 0, 0, 0, 0/) !me
...
!!$      allvals(:,1,30) = (/     -14593,      33923, 0, 0, 0, 0, 0, 0/) !tt
!!$      allvals(:,1,31) = (/     -42929,      40758, 0, 0, 0, 0, 0, 0/) !uu
!!$      allvals(:,1,32) = (/     -19920,      23361, 0, 0, 0, 0, 0, 0/) !vv
...
!!$      dateo_S = '20081215.000000'
      ...
      if (.not.RMN_IS_OK(index)) then
         call msg(MSG_ERROR,'test_input2 - cannot init input_mod for: '//trim(F_filename_S))
         return
      endif
      ...
      STEPLOOP: do istep = 0,NSTEP
         ...
         VARLOOP: do ivar=1,nbvar
            ...
            if (.not.(RMN_IS_OK(istat) .and. associated(data))) then
               call msg(MSG_WARNING,'test_input2 - var not found: '//trim(varname_S))
...
!!$                    print *,'ERROR v2',istep,ii,varname2_S(1:8),index
      ...
      call testutils_assert_ok(all(result_S==result2_S),'var list')
      ...
      do istep = 0,NSTEP
         do ii=1,NVAR
            ...
            call testutils_assert_ok(ok_L(istep,ii),'values for "'//trim(result_S(istep,ii))//'" at step='//trim(dummy_S))   
         enddo
      enddo
