...
#include <rmn/WhiteBoard.hf>
   ...
   call testutils_set_name('test_ptopo_mpi')
   ...
   call testutils_assert_ok(.not.RMN_IS_OK(istat),'test_ptopo_mpi','collect_dims not possible')


   !- collect bloc 3d
   istat = ptopo_collect_dims(RPN_COMM_BLOC_COMM,NI0,NJ0,bni,bnj,i0,j0)
   call testutils_assert_ok(RMN_IS_OK(istat),'test_ptopo_mpi','collect_dims bloc')
   ok_L = (&
        bni == NI0 * ptopo_bloc_npex .and. &
        bnj == NJ0 * ptopo_bloc_npey  .and. &
        i0  == 1 + NI0 * ptopo_bloc_ipex.and. &
        j0  == 1 + NJ0 * ptopo_bloc_ipey)
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','collect_dims bloc data')
   if (.not.ok_L) then
...
!!         print *,'in ',i,j,ptopo_bloc_ipex,ptopo_bloc_ipey,datain(i,j,1)
   ...
   call testutils_assert_ok(RMN_IS_OK(istat),'test_ptopo_mpi','bloc_collect')
   ok_L = .true.
   if (ptopo_isblocmaster_L) ok_L = associated(dataout)
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','bloc_collect associated')
   if (ptopo_isblocmaster_L .and. ok_L) ok_L = all(ubound(dataout)==(/bni,bnj,NK0/))
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','bloc_collect dims')
   ...
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','bloc_collect data')
   ...
   call testutils_assert_ok(RMN_IS_OK(istat),'test_ptopo_mpi','collect_dims grid')
   ok_L = (&
        bni == NI0 * ptopo_grid_npex .and. &
        bnj == NJ0 * ptopo_grid_npey  .and. &
        i0  == 1 + NI0 * ptopo_grid_ipex.and. &
        j0  == 1 + NJ0 * ptopo_grid_ipey)
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','collect_dims grid data')
   if (.not.ok_L) then
      print *,'bni,bnj,i0,j0',bni,bnj,i0,j0
   endif
...
!!         print *,'in ',i,j,ptopo_bloc_ipex,ptopo_bloc_ipey,datain(i,j,1)
   ...
   call testutils_assert_ok(RMN_IS_OK(istat),'test_ptopo_mpi','grid_collect')
   ok_L = .true.
   if (ptopo_grid_ipe == RPN_COMM_MASTER) ok_L = associated(dataout)
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect associated')

   if (ptopo_grid_ipe == RPN_COMM_MASTER .and. ok_L) ok_L = all(ubound(dataout)==(/bni,bnj,NK0/))
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect dims')
   ...
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect data')
   ...
   call testutils_assert_ok(RMN_IS_OK(istat),'test_ptopo_mpi','grid_collect 2d')
   ok_L = .true.
   if (ptopo_grid_ipe == RPN_COMM_MASTER) ok_L = associated(dataout2d)
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect associated 2d')
   if (ptopo_grid_ipe == RPN_COMM_MASTER .and. ok_L) ok_L = all(ubound(dataout2d)==(/bni,bnj/))
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect dims 2d')
   if (ptopo_grid_ipe == RPN_COMM_MASTER .and. ok_L) then
      do j=1,bnj
         do i=1,bni
            tmp_r = float(i)+float(j)/100.
   ...
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect data 2d')
   ...
   istat = ptopo_collect(dataout2d,datain2d,RPN_COMM_GRID,i0,j0,NI0,NJ0)
   call testutils_assert_ok(RMN_IS_OK(istat),'test_ptopo_mpi','grid_collect 2dh')
   ok_l = .true.
   if (ptopo_grid_ipe == RPN_COMM_MASTER) ok_L = associated(dataout2d)
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect associated 2dh')
   if (ptopo_grid_ipe == RPN_COMM_MASTER .and. ok_L) ok_L = all(ubound(dataout2d)==(/bni,bnj/))
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect dims 2dh')
   if (ptopo_grid_ipe == RPN_COMM_MASTER .and. ok_L) then
      do j=1,bnj
         do i=1,bni
            tmp_r = float(i)+float(j)/100.
   ...
   call testutils_assert_ok(ok_L,'test_ptopo_mpi','grid_collect data 2dh')
