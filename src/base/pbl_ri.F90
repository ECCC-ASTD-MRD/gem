...
contains
  ...
  subroutine rpnint(F_en, F_km, F_kt, F_pri, F_rif, F_rig, F_buoy, F_n2, F_shr2, &
       F_buoyen, F_shren, F_diffen, F_dissen, F_qwvar, F_zn, F_znt, F_znu, F_znd, F_zd, &
       ...
       F_sigm, F_sigt, F_gzm, F_gzt, F_dxdy, F_std_p_prof, F_mrk2, F_vcoef, &
...
#include "phymkptr.hf"
    ...
    real, dimension(F_ni), intent(in) :: F_std_p_prof           !standard pressure profile (Pa)
    ...
    real, dimension(F_ni,F_nkm1), intent(inout) :: F_znt        !thermal mixing length (m)
    real, dimension(F_ni,F_nkm1), intent(inout) :: F_n2         !buoyancy frequency squared (s^-2)
    real, dimension(F_ni,F_nkm1), intent(out) :: F_znu          !upwards momentum mixing length (m)
    real, dimension(F_ni,F_nkm1), intent(out) :: F_znd          !downwards momentum mixing length (m)
    ...
    real, dimension(F_ni,F_nkm1), intent(inout) :: F_shr2       !square of vertical wind shear (s^-2)
    ...
    if (F_kount == 0) then
       ...
       if (.not.ISPHYIN('znt')) F_znt = F_zn
    endif

    ! Estimate buoyancy and shear gradients
    call buoyflux(F_n2, F_shr2, F_rig, F_uu, F_vv, F_tt, F_hu, &
         F_lwc, F_iwc, F_fbl, F_tsurf, F_qsurf, F_z0m, F_z0t, &
         F_lat, F_fcor, F_sigt, F_gzm, F_gzt, F_ps, F_vcoef, &
         F_tau, F_ni, F_nkm1)

    ! Initialize the turbulence regime
    if (F_kount == 0) then
       call turbreg(F_turbreg, hyst, F_rig, F_sigt, F_ps, F_mrk2, &
            F_ni, F_nkm1, F_init=.true.)
       if (phy_error_L) return
    endif
    
    ! Compute mixing and dissipation length scales
    mlen = ilongmel
    stat = ml_compute(F_zn, F_znt, F_zd, F_pri, mlen, F_tt, F_hu, F_lwc, F_iwc, F_fbl, &
         F_gzt, F_gzm, F_gzm, F_sigt, F_sigm, F_sigm, F_ps, F_enold, F_n2, &
         F_rig, zero3d, zero, F_turbreg, F_z0m, F_hpbl, F_lh, F_hpar, F_std_p_prof, &
         F_vcoef, F_mrk2, F_dxdy, F_tau, F_kount, znu=F_znu, znd=F_znd)
    if (stat /= PHY_OK) then
    ...
    do k=1,F_nkm1
       do i=1,F_ni          
          F_buoy(i,k) = F_pri(i,k) * F_n2(i,k)
          F_rif(i,k) = F_pri(i,k) * F_rig(i,k)
       enddo
    enddo

    ! Update the turbulence regime
    call turbreg(F_turbreg, hyst, F_rif, F_sigt, F_ps, F_mrk2, F_ni, F_nkm1)
    
    ! Solve the algebraic part of the TKE equation
    do k=1,F_nkm1
       do i=1,F_ni
          b_term(i,k) = hyst(i,k) * PBL_RI_CK * F_zn(i,k) * (F_shr2(i,k) - F_buoy(i,k))
