...
#include <rmn/msg.h>
   ...
   call msg_toall(chm_msg_debug, 'chm_cffeps_init [BEGIN]')
!
   chm_cffeps_init = -1
...
!
   ...
   if (local_dbg) then
      write(chm_lun_out, *)' -- for processor ', my_pe, ' chm_do_cffeps_l is ', chm_do_cffeps_l
      write(chm_lun_out, *)' -- for processor ', my_pe, ' RPN_COMM_spread returned ', lc_hotspots , ' sources'
   end if
...
!***********************************************************************
   chm_cffeps_init = 1
   call msg_toall(chm_msg_debug, 'chm_cffeps_init [END]')
   return
