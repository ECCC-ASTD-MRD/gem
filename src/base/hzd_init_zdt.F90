...
!
      ...
      allocate( a_zdt(l_minx:l_maxx, l_miny:l_maxy,l_nk),&
               b_zdt(l_minx:l_maxx, l_miny:l_maxy,l_nk), &
               c_zdt(l_minx:l_maxx, l_miny:l_maxy,l_nk), &
               W_zdt(l_minx:l_maxx, l_miny:l_maxy,l_nk) )
      ...
      enddo
         ...
         do j=1+pil_s, l_nj-pil_n
            do i=1+pil_w, l_ni-pil_e
                c_zdt(i,j,k)= -beta_imp*vsten(i,j,3,k )
                b_zdt(i,j,k)=one-beta_imp*vsten(i,j,1,k)
            enddo
         enddo
         do k=2,l_nk-1
            do j=1+pil_s, l_nj-pil_n
               do i=1+pil_w, l_ni-pil_e
                  a_zdt(i,j,k)= -beta_imp*vsten(i,j,2,k)
                  b_zdt(i,j,k)= one-beta_imp*vsten(i,j,1,k)
                  c_zdt(i,j,k)= -beta_imp*vsten(i,j,3,k)
               enddo
            enddo
         ...
         do j=1+pil_s, l_nj-pil_n
            do i=1+pil_w, l_ni-pil_e
               do k = 2 , l_nk
                  W_zdt(i,j,k) = a_zdt(i,j,k) / b_zdt(i,j,k - 1)
                  b_zdt(i,j,k) = b_zdt(i,j,k) - W_zdt(i,j,k) * c_zdt(i,j,k - 1)
               enddo
            enddo
         enddo
