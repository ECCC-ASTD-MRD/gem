...
!                  call per step via rpnphy base/phyexe
...
!!if_off
   use chm_utils_mod,          only: chm_lun_out, undefined, &
...
!
   ...
   write(tmp_S, '(a,i4,a,i3,a,i3,a,i3,a)') 'chm_exe [BEGIN] step =', step, &
         ' chm_ni=', chm_ni, ' chm_nk=', chm_nk, ' slab= ', slab_index
   call msg_toall(chm_msg_debug, trim(tmp_S))
...
!
   ...
   if (mod(step, chm_step_factor) == 0) then
      ...
      call chm_load_store_tracers(pvars, chem_tr, 0)
      ...
      if (chm_model_s(1:4) == 'MACH') then
         ...
         call mach_main(pvars, chem_tr, metvar2d, metvar3d, slab_index, &
...
!
      call chm_load_store_tracers(pvars, chem_tr, 1)
