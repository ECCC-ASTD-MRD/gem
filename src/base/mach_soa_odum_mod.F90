...
!===============================================================================
 CONTAINS
   ...
   subroutine map_soa_dvoc_svoc_saprc(dvoc_ppm, dvoc_soa, tt, gni, gnk, conx1)
...
!! convert input dvoc_gas from ppmv to ug/m3 and output to dvoc_soa
      ...
      real(dp),        intent (in) :: conx1(gni * gnk) !ppmv to molec/cm3 factor
...
! local variables
 ...
 !   mach_pkg_saprc_mod.ftn90
      integer(kind=4) :: gas_alk4, gas_alk5, gas_ole1, gas_ole2, gas_aro1, &
                         ...
                         gas_sesq, gas_ivoc, i, k, jj, isp, nn
      ...
      do jj = 1, nsp_soa_gases
         ...
         do k = nk_start, gnk
            do i = 1, gni
               nn = nn + 1
               dvoc_gas(jj, i, k) = dvoc_ppm(jj, i, k) * conx1(nn) / avno * &
                                     sm(gas_species(isp))%mol_wt * 1.D+12
            end do
         end do
      end do
