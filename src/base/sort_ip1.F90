...
subroutine record_sort_ip1 ( F_keys, F_ip1, F_nka )
   use rmn_fst24

   implicit none

   integer         , intent(inout) :: F_nka
   integer         , intent(out)   :: F_ip1 (F_nka)
   type(fst_record), intent(inout) :: F_keys(F_nka)
   type(fst_record)                :: keys(F_nka),diag_key
...
#include <rmnlib_basics.hf>
   ...
   character(len=4)  :: var
   character(len=12) :: etik_S
   integer i,j,k,cnt,err,diag_ip1
   integer kind, res
   real ip1r4(F_nka),lev,temp_ip1(F_nka),hugetype
   integer liste(F_nka),ip1(F_nka)
   !
   ! --------------------------------------------------------------------
   !
   cnt= 0 ; diag_ip1= 0 ; kind= -1
   ...
   do k= 1, F_nka
      call convip_plus(F_keys(k)%ip1, lev, i,-1, blk_S, .false.)
      if ( i /= 4 ) then
         cnt= cnt + 1
         ip1r4(cnt)= lev
         keys (cnt)= F_keys(k)
         ip1  (cnt)= F_keys(k)%ip1
         kind = i
      else
         diag_key  = F_keys(k)
         diag_ip1  = F_keys(k)%ip1
      endif
   end do
   ...
   !
   return
end subroutine record_sort_ip1
