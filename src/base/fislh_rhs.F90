         ...
         do i= 1, l_ni
            ...
            orhsu_ext(i,j,k) = invT_m_8  * ut1(i,j,k) - Cstv_Beta_m_8 * ( &
                         ...
                         + Cstv_Beta_m_8 * t_interp * GVM%mc_Jx_8(i,j,k) * ( &
                           Ver_wp_8%m(k)*half*( (qt1(i+1,j,k+1)-qt1(i+1,j,k ))*GVM%mc_iJz_8(i+1,j,k )   &
                                              + (qt1(i  ,j,k+1)-qt1(i  ,j,k ))*GVM%mc_iJz_8(i  ,j,k ) ) &
                         + Ver_wm_8%m(k)*half*( (qt1(i+1,j,k  )-qt1(i+1,j,km))*GVM%mc_iJz_8(i+1,j,km)   &
                                              + (qt1(i  ,j,k  )-qt1(i  ,j,km))*GVM%mc_iJz_8(i  ,j,km) ) )&
            ...
            orhsv_ext(i,j,k) = invT_m_8  * vt1(i,j,k) - Cstv_Beta_m_8 * ( &
                         ...
                         + Cstv_Beta_m_8 * t_interp * GVM%mc_Jy_8(i,j,k) * ( &
                           Ver_wp_8%m(k)*half*( (qt1(i,j+1,k+1)-qt1(i,j+1,k ))*GVM%mc_iJz_8(i,j+1,k )   &
                                              + (qt1(i,j  ,k+1)-qt1(i,j  ,k ))*GVM%mc_iJz_8(i,j  ,k ) ) &
                         + Ver_wm_8%m(k)*half*( (qt1(i,j+1,k  )-qt1(i,j+1,km))*GVM%mc_iJz_8(i,j+1,km)   &
                                              + (qt1(i,j  ,k  )-qt1(i,j  ,km))*GVM%mc_iJz_8(i,j  ,km) ) )&
            ...
            div = (ut1 (i,j,k)-ut1 (i-1,j,k))*geomh_invDXM_8(j)     &
                   ...
                   + half * ( GVM%mc_Ix_8(i,j,k)*(ut1(i,j,k)+ut1(i-1,j,k))        &
                   + GVM%mc_Iy_8(i,j,k)*(vt1(i,j,k)+vt1(i,j-1,k)) )      &
                   + GVM%mc_Iz_8(i,j,k)*(Ver_wp_8%m(k)*zdt1(i,j,k)+Ver_wm_8%m(k)*Ver_onezero(k)*zdt1(i,j,km))

            orhsc_ext (i,j,k) = invT_8 *  w4 * qt1(i,j,k) +   invT_8 * GVM%mc_logJz_8(i,j,k)  &
         ...
         do n= HLT_start, HLT_end
            ...
            do i= 1, l_ni
               ...
               orhsw_ext(i,j,k) = invT_nh_8 * wt1(i,j,k) - Cstv_Beta_nh_8 *w2* &
                              ( (qt1(i,j,k+1)-qt1(i,j,k))*GVM%mc_iJz_8(i,j,k)  -grav_8*(one-one/w2) )
            end do
         end do
      endif
