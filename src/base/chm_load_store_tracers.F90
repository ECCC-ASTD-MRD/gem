...
!
!!if_on
subroutine chm_load_store_tracers(pvars, chem_tr, flag)
   use phymem,               only: phyvar
   use chm_species_info_mod, only: nb_dyn_tracers
   use chm_ptopo_grid_mod,   only: chm_ni, chm_nk
!!if_off
   use chm_species_info_mod, only: species_master
   use chm_utils_mod,        only: chm_error_l, ik, chm_msg_debug
   implicit none
!!if_on
...
! Local Variables
!
   integer(kind=4) :: isp, ii, kk, this_ik, busid
!
   ...
   if (flag == 0) then
    ! Copy tracers concentrations from the dynamic bus
      do isp = 1, nb_dyn_tracers
         if (species_master(isp)%dyn_pvarid > 0) then
            busid = species_master(isp)%dyn_pvarid
            do kk = 1, chm_nk + 1
               do ii = 1, chm_ni
                  this_ik = ik(ii, kk, chm_ni) + 1
                  chem_tr(ii, kk, isp) = pvars(busid)%data(this_ik)
               end do
            end do
         else
            chem_tr(:, :, isp) = 0.0
      ...
      do isp = 1, nb_dyn_tracers
         if (species_master(isp)%dyn_pvarid > 0) then
            ...
            do kk = 1, chm_nk + 1
               do ii = 1, chm_ni
                  this_ik = ik(ii, kk, chm_ni) + 1
                  pvars(busid)%data(this_ik) = chem_tr(ii, kk, isp)
               end do
            end do
         end if
      end do
