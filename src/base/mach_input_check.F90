...
!                 OUT
!                     pvars, or KT_NEW     => phybus pointer for vertical diffusion for GEM-MACH
!
!============================================================================
!
!!if_on
...
!
!  Debugging
   call msg_toall(chm_msg_debug, 'mach_input_check [BEGIN]')
      ...
      do k = 1, chm_nk
         do i = 1, chm_ni
            if (chm_debug_2d_i >= 1 .and. k == chm_nk) &
               pvars(sm(dbg_2d(1))%out_pvarid)%data(i) = landuse(i,15)
...
!
   ...
   do k = 1, chm_nk
      do i = 1, chm_ni
         pvars(sm(sp_KTN)%out_pvarid)%data(ik(i, k, chm_ni) + 1) = kt_new(i, k)
      end do
   end do

   call msg_toall(chm_msg_debug, 'mach_input_check [END]')
