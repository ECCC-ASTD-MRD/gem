...
!     ---------------------------------------------------------------
      ...
      rad2deg_8 = 180.d0 / pi_8 !Should not need this..
      deg2rad_8 = pi_8 / 180.d0
      ...
      geomh_invDY_8  = 1.d0 / ( Dcst_rayt_8 * Del_yg )
      ...
      do j = 1 - G_haloy, l_nj + G_haloy
         ...
         geomh_invDX_8  (j) = 1.d0 / (scale_factor   * Del_xg )
         geomh_invDXv_8(j)  = 1.d0 / (scale_factor_v * Del_xg )
      ...
      if ( .not. Ctrl_theoc_L) then
        ...
        ! Why set this to zero before setting it again with the result of
        ! a function call?  Can the function return without a value?
        ! If so, WTF Fortran!?
        glbphycore_gid = 0
      ...
      do j = 1, l_nj
         do i = 1, l_ni
            if (geomh_lonrx(i,j) >= 180.0) then
               geomh_lonrx(i,j) = geomh_lonrx(i,j) - 360.0
            end if
         end do
      end do
