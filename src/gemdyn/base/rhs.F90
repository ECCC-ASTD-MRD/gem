...
!
      ...
      real(kind=REAL64) :: tdiv, BsPqbarz, barz, barzp,  &
                u_interp, v_interp, t_interp, mu_interp, w1, w2
...
!********************************************
! Compute Ru: RHS of U and V equation       *
      ...
      do k = 1,l_nk
         ...
         do j=2-G_haloy,l_nj+G_haloy-1
            do i=2-G_halox,l_ni+G_halox-1
               ...
               w2=log(F_t(i,j,k) / Cstv_Tstr_8)
               BsPqbarz = Ver_wp_8%t(k)*w1+Ver_wm_8%t(k)*BsPq(i,j,k)
               F_ort(i,j,k) = Cstv_invT_8 * ( w2 - Cstv_bar1_8 * cappa_8 * BsPqbarz ) &
               ...
               w1= log(one + Ver_dbdz_8%m(k) * F_s(i,j) &
