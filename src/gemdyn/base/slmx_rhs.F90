...
!---------------------------------- LICENCE END ---------------------------------
!
!*s/r slmx_rhs - compute the right-hand sides

      subroutine slmx_rhs ( F_dt_8 )
      use HORgrid_options
      use gem_options
...
!$omp do collapse(2)
      do k=1, l_nk
         do j=1, l_nj
         do i= 1, l_ni
            logT(i,j,k)= log (tt2(i,j,k)/Cstv_Tstr_8)
            logP(i,j,k)= log ((qt2(i,j,k) - fis0(i,j))*Cstv_invFI_8 + 1.d0)
        end do
      end do
      end do
!$omp enddo
...
!**********************************************
      ...
      do n= HLT_start, HLT_end
         ...
         do i= 1, l_ni
            ...
            barzp = Ver_wp_8%m(k)*tt2(i+1,j,k)+Ver_wm_8%m(k)*tt2(i+1,j,km)
            t_interp = (barz + barzp)*half/Cstv_Tstr_8
            v_interp = 0.25d0*(vt2(i,j,k)+vt2(i,j-1,k)+vt2(i+1,j,k)+vt2(i+1,j-1,k))
            ...
            barz  = Ver_wp_8%m(k)*tt2(i,j  ,k)+Ver_wm_8%m(k)*tt2(i,j  ,km)
            barzp = Ver_wp_8%m(k)*tt2(i,j+1,k)+Ver_wm_8%m(k)*tt2(i,j+1,km)
            t_interp = ( barz + barzp)*half/Cstv_Tstr_8
            u_interp = 0.25d0*(ut2(i,j,k)+ut2(i-1,j,k)+ut2(i,j+1,k)+ut2(i-1,j+1,k))
            ...
            orhsc_ext (i,j,k) = invT_8 *  w4 * qt2(i,j,k) +   invT_8 * mc_logJz_8(i,j,k)  &
                          ...
                          Ver_onezero(k)*Ver_wm_8%m(k)*phy_tv_tend(i,j,km)/tt2(i,j,km) ) &
