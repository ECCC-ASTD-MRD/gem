...
!**s/r adz_BC_LAM_Aranami - Estimate FLUX_out/FLUX_in based on Aranami et al. (2015)
      ...
      subroutine adz_od_BC_LAM_Aranami (F_stk,F_xyz,F_export,F_num_io,F_k0_io,F_minx,F_maxx,F_miny,F_maxy,F_post,F_nptr)
...
#include <arch_specific.hf>
      ...
      !arguments
      !---------
      integer, intent(in) :: F_num_io,F_k0_io,F_minx,F_maxx,F_miny,F_maxy,F_nptr
      ...
      real, dimension(F_minx:F_maxx,F_miny:F_maxy,l_nk,F_nptr), target :: adv_o,adv_i
...
!---------------------------------------------------------------------
!
      i0 = 1-G_halox    + G_halox*west
      in = l_ni+G_halox - G_halox*east
      j0 = 1-G_haloy    + G_haloy*south
      jn = l_nj+G_haloy - G_haloy*north
      ...
      if (F_nptr<=MAXTR3D) then !Treat Tracer in Tr3d_list
         ...
         do n=1,F_nptr
            ...
            do j=j0,jn
            do i=i0,in
               adv_o(i,j,k,nf) = Adz_BC_LAM_mask_o(i,j,k) * F_stk(n)%src(i,j,k)
               adv_i(i,j,k,nf) = Adz_BC_LAM_mask_i(i,j,k) * F_stk(n)%src(i,j,k)
            end do
            end do
            end do
      ...
      call adz_od_corflux (Adz_flux,stkpntr,nptr_F,Adz_cpntr_t,F_export,tr_num,F_nptr)
