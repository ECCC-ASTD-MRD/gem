...
!**s/r  pre_jacobi_hlt -  BlocJacobi_3D additive-Schwarz preconditioner
!          with local solver = "separable" approximate elliptic problem

      subroutine pre_jacobi_hlt ( Sols,Rhs_b,Ni,Nj,Nk)
...
!!$            DO l = 1,nk
!!$               do i = 1, ni
!!$                  w2_8 (i,j,k) = w2_8 (i,j,k) + Opr_lzevec_8((k-1)*Nk+l)*rhs_b(i,j,l)
!!$               end do
!!$            end do
!!$            do i = 1, ni
!!$               ii= Sol_ii0+i+l_i0 - 2
