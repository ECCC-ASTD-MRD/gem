...
! eps  EXP(-dt/tau/2.146)
      ...
      integer :: itstep_s, iperiod_iau, err, ier ,unf, nch2d, spp_indx, stat
            ...
            if (ptopo_couleur == 0  .and. ptopo_myproc == 0) then
               do nc=1,nch2d
                  ...
                  if (ier == 0) then
                     ...
                     do i=1,36
                        read (unf)dumdum(i,np)
                     end do
                     do l=lmin,lmax
                        do m=1,l+1
                           read(unf)ar_p(lmax-l+1,m,nc)
                           read(unf)ai_p(lmax-l+1,m,nc)
                           read(unf)br_p(lmax-l+1,m,nc)
                           read(unf)bi_p(lmax-l+1,m,nc)
                        end do
                     end do
                     ier=fclos(unf)
                  else
...
! Save random numbers and coefficient ar,ai,br,bi
         if (write_markov_l) then
            ...
            if (ptopo_couleur == 0  .and. ptopo_myproc == 0) then
               ...
               ier=fnom(unf,fn,'SEQ+UNF+NEW',0)

               if ( ier == 0 ) then
                  write(output_unit,2000) 'WRITING', trim(fn)
                  do i=1,36
                     write (unf)dumdum(i,np)
                  end do
                  do l=lmin,lmax
                     do m=1,l+1
                        write(unf)ar_p(lmax-l+1,m,nc)
                        write(unf)ai_p(lmax-l+1,m,nc)
                        write(unf)br_p(lmax-l+1,m,nc)
                        write(unf)bi_p(lmax-l+1,m,nc)
                     end do
                  end do
                  ier=fclos(unf)
               else
