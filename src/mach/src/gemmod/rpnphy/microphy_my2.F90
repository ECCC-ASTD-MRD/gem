...
! !    endif
      ...
      if (present(massFlux1D)) then
         massFlux1D = 0.0
         do k= kbot,ktop,kdir
            massFlux1D(k)= VVQ(k)*DE1d(k)*QX1d(k)
         enddo
      endif
     !--
...
!==
 ...
 do k = ktop-kdir,kbot,-kdir
    do i = 1,ni
       tmp1= QR(i,k) + QI(i,k) + QN(i,k) + QG(i,k) + QH(i,k) + SS(i,k,4)
       if (tmp1 > 0.) then
          SS(i,k,4) = SS(i,k,4)/(tmp1 + epsQ)
       else
          SS(i,k,4) = 0.
       endif
     enddo
...
!====
  ...
  !-- Diagnostics for chemistry module: precipitation fluxes
   do k=ktop,kbot,-kdir
      do i=1,ni
         SS(i,k,1) = massFlux3D_r(i,k)
         SS(i,k,2) = massFlux3D_i(i,k) + massFlux3D_s(i,k) + massFlux3D_g(i,k) + massFlux3D_h(i,k)
      enddo
   enddo
  !==
