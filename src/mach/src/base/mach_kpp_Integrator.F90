...
!
! GEM-MACH Setup
! Fichier / File   : mach_kpp_Integrator.ftn90
! Creation         : Jack Chen (Feb. 2016), Diane Pendlebury (Feb. 2018) and
!                    Deji Akingunola (Nov. 2020)
! Modifications    : code adapted from KPP
!   modified for inclusion to GEMMACH fixed parameters are from modules
!    but concentrations and reaction coefficient arrays (RCONST,FIX)
!    are now passed as routine arguments to avoid OMP>1 thread-safety
!    issues
!
!  - Modified two function to not call "Update_Sun()" and "Update_RCONST()"
!    as reaction coefficients are input from "mach_gas_main_xx"
!  "FunTemplate( T, Y, Ydot )" -> FunTemplate( Y, Ydot, FIX, RCONST )
!  "JacTemplate( T, Y, Jcb )" -> JacTemplate( Y, Jcb, FIX, RCONST )
!  "ros_FunTimeDerivative(T,Roundoff,Y,Fcn0,dFdT)" -> ros_FunTimeDerivative(T,Roundoff,Y,Fcn0,dFdT,FIX,RCONST)
!
!  - Arrays (RCONST,FIX) are input to "Rosenbrock" routine and updated
!    as variable in the module to be used by other subroutines within the
!    "Integrator" modules and functions
!
!  - add debug flag and change print statements to write (chm_lun_out, *)
!
!  - utilize the Intel MKL calls WAXPY->DAXPY; WSCAL->DSCAL
!
!  - rename to mach_kpp_integrator, and use procedural pointers to consolidate the various
!    KPP integrator subroutines for ADOM2, SAPRC07C, SAPRC07CS and SAPRC11 into a single
!    common routine; to simplify future updates and development.
!============================================================================
!
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
...
!!if_off
  ...
  use mach_pkg_gas_mod, only: nvar, lu_nonzero, minkppconc
  use chm_nml_mod,      only: chm_pkg_gas_s
  use mach_kpp_adom2_Function,          only: adom2_Fun => Fun
  use mach_kpp_adom2_Jacobian,          only: adom2_Jac_SP => Jac_SP
  use mach_kpp_adom2_LinearAlgebra,     only: adom2_KppDecomp => KppDecomp, &
                                              adom2_KppSolve => KppSolve
  use mach_kpp_adom2_JacobianSP,        only: adom2_LU_DIAG => LU_DIAG, &
                                              adom2_LU_ICOL => LU_ICOL, &
                                              adom2_LU_IROW => LU_IROW
  use mach_kpp_saprc07c_Function,       only: s07c_Fun => Fun
  use mach_kpp_saprc07c_Jacobian,       only: s07c_Jac_SP => Jac_SP
  use mach_kpp_saprc07c_LinearAlgebra,  only: s07c_KppDecomp => KppDecomp, &
                                              s07c_KppSolve => KppSolve
  use mach_kpp_saprc07c_JacobianSP,     only: s07c_LU_DIAG => LU_DIAG, &
                                              s07c_LU_ICOL => LU_ICOL, &
                                              s07c_LU_IROW => LU_IROW
  use mach_kpp_saprc07cs_Function,      only: s07cs_Fun => Fun
  use mach_kpp_saprc07cs_Jacobian,      only: s07cs_Jac_SP => Jac_SP
  use mach_kpp_saprc07cs_LinearAlgebra, only: s07cs_KppDecomp => KppDecomp, &
                                              s07cs_KppSolve => KppSolve
  use mach_kpp_saprc07cs_JacobianSP,    only: s07cs_LU_DIAG => LU_DIAG, &
                                              s07cs_LU_ICOL => LU_ICOL, &
                                              s07cs_LU_IROW => LU_IROW
!
  implicit none
...
!~~~>  GEM-MACH procedural interface for abstracting various mechanisms functions
   abstract interface
      subroutine Func (V, F, RCT, Vdot)
         use chm_utils_mod,    only: dp
      ...
      subroutine JacSP (V, F, RCT, JVS)
         use chm_utils_mod,    only: dp
         use mach_pkg_gas_mod, only: nvar, nfix, nreact, lu_nonzero
         REAL(kind=dp) :: V(NVAR)
         REAL(kind=dp) :: F(NFIX)
         REAL(kind=dp) :: RCT(NREACT)
         REAL(kind=dp) :: JVS(LU_NONZERO)
      end subroutine JacSP
      subroutine Decomp (JVS, IER)
         use chm_utils_mod,    only: dp
         use mach_pkg_gas_mod, only: lu_nonzero
         INTEGER       :: IER
         REAL(kind=dp) :: JVS(LU_NONZERO)
      end subroutine Decomp
      subroutine rSolve (JVS, X)
         use chm_utils_mod,    only: dp
   ...
   procedure(Func),       pointer :: Fun_ptr => null ()
   procedure(JacSP),      pointer :: Jac_SP_ptr => null ()
   procedure(Decomp),     pointer :: KppDecomp_ptr => null ()
   procedure(rSolve),     pointer :: KppSolve_ptr => null ()
   integer, dimension(:), pointer :: LU_DIAG_ptr, LU_ICOL_ptr, LU_IROW_ptr
...
!~~~>   Parameters
   ...
   if (chm_pkg_gas_s(1:9) == 'SAPRC07CS') then
      Fun_ptr => s07cs_Fun
      Jac_SP_ptr => s07cs_Jac_SP
      KppDecomp_ptr => s07cs_KppDecomp
      KppSolve_ptr => s07cs_KppSolve
      LU_DIAG_ptr => s07cs_LU_DIAG
      LU_ICOL_ptr => s07cs_LU_ICOL
      LU_IROW_ptr => s07cs_LU_IROW
   else if (chm_pkg_gas_s(1:8) == 'SAPRC07C') then
      Fun_ptr => s07c_Fun
      Jac_SP_ptr => s07c_Jac_SP
      KppDecomp_ptr => s07c_KppDecomp
      KppSolve_ptr => s07c_KppSolve
      LU_DIAG_ptr => s07c_LU_DIAG
      LU_ICOL_ptr => s07c_LU_ICOL
      LU_IROW_ptr => s07c_LU_IROW
   else if (chm_pkg_gas_s(1:8) == 'ADOM2KPP') then
      Fun_ptr => adom2_Fun
      Jac_SP_ptr => adom2_Jac_SP
      KppDecomp_ptr => adom2_KppDecomp
      KppSolve_ptr => adom2_KppSolve
      LU_DIAG_ptr => adom2_LU_DIAG
      LU_ICOL_ptr => adom2_LU_ICOL
      LU_IROW_ptr => adom2_LU_IROW
   end if
...
!~~~>   Initialize the particular Rosenbrock method selected
   SELECT CASE (ICNTRL(3))
     ...
     CASE (6)
       CALL Rang3
     CASE DEFAULT
       if (chm_lun_out > 0) write(chm_lun_out, *) 'Unknown Rosenbrock method: ICNTRL(3)=',ICNTRL(3)
...
!~~~>   The maximum number of steps admitted
   ...
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected max no. of steps: ICNTRL(4)=',ICNTRL(4)
...
!~~~>  Lower bound on the step size: (positive value)
   ...
   ELSEIF (RCNTRL(1) > ZERO) THEN
      Hmin = RCNTRL(1)
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected Hmin: RCNTRL(1)=', RCNTRL(1)
...
!~~~>  Upper bound on the step size: (positive value)
   ...
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected Hmax: RCNTRL(2)=', RCNTRL(2)
      CALL ros_ErrorMsg(-3,Tstart,ZERO,IERR)
      RETURN
   END IF
!~~~>  Starting step size: (positive value)
   ...
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected Hstart: RCNTRL(3)=', RCNTRL(3)
      CALL ros_ErrorMsg(-3,Tstart,ZERO,IERR)
      RETURN
   END IF
!~~~>  Step size can be changed s.t.  FacMin < Hnew/Hold < FacMax
   ...
   ELSEIF (RCNTRL(4) > ZERO) THEN
      FacMin = RCNTRL(4)
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected FacMin: RCNTRL(4)=', RCNTRL(4)
   ...
   ELSEIF (RCNTRL(5) > ZERO) THEN
      FacMax = RCNTRL(5)
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected FacMax: RCNTRL(5)=', RCNTRL(5)
      CALL ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
!~~~>   FacRej: Factor to decrease step after 2 succesive rejections
   ...
   ELSEIF (RCNTRL(6) > ZERO) THEN
      FacRej = RCNTRL(6)
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected FacRej: RCNTRL(6)=', RCNTRL(6)
      CALL ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
!~~~>   FacSafe: Safety Factor in the computation of new step size
   ...
   ELSEIF (RCNTRL(7) > ZERO) THEN
      FacSafe = RCNTRL(7)
   ELSE
      if (chm_lun_out > 0) write(chm_lun_out, *) 'User-selected FacSafe: RCNTRL(7)=', RCNTRL(7)
      CALL ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
!~~~>  Check if tolerances are reasonable
    DO i=1,UplimTol
      IF ( (AbsTol(i) <= ZERO) .OR. (RelTol(i) <= 10.0_dp*Roundoff) &
        ...
        if (chm_lun_out > 0) write(chm_lun_out,*) ' AbsTol(',i,') = ',AbsTol(i)
        if (chm_lun_out > 0) write(chm_lun_out,*) ' RelTol(',i,') = ',RelTol(i)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 CONTAINS !  SUBROUTINES internal to Rosenbrock (mach_kpp_Integrator)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   SELECT CASE (Code)
    CASE (-1)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> Improper value for maximal no of steps'
    CASE (-2)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> Selected Rosenbrock method not implemented'
    CASE (-3)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> Hmin/Hmax/Hstart must be positive'
    CASE (-4)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> FacMin/FacMax/FacRej must be positive'
    CASE (-5)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> Improper tolerance values'
    CASE (-6)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> No of steps exceeds maximum bound'
    CASE (-7)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> Step size too small: T + 10*H = T', &
            ' or H < Roundoff'
    CASE (-8)
      if (chm_lun_out > 0) write(chm_lun_out, *) '--> Matrix is repeatedly singular'
    CASE DEFAULT
      if (chm_lun_out > 0) write(chm_lun_out, *) 'Unknown Error code: ', Code
   END SELECT

  if (chm_lun_out > 0) write(chm_lun_out, *) "T=", T, "and H=", H
...
!~~~> Input: tolerances
   REAL(kind=dp), INTENT(IN) ::  AbsTol(N), RelTol(N)
!~~~> Rx Rate and Fix Species
   REAL(kind=dp), INTENT(IN) ::  RCONST(NREACT)
   REAL(kind=dp), INTENT(IN) ::  FIX(NFIX)
...
!~~~>   Compute the function at current time
   CALL FunTemplate(Y,Fcn0,FIX,RCONST)
   ISTATUS(Nfun) = ISTATUS(Nfun) + 1

!~~~>  Compute the function derivative with respect to T
   IF (.NOT.Autonomous) THEN
      CALL ros_FunTimeDerivative (T, Roundoff, Y, &
                Fcn0, dFdT, FIX, RCONST )
   END IF

!~~~>   Compute the Jacobian at current time
   CALL JacTemplate(Y,Jac0,FIX,RCONST)
...
Stage: DO istage = 1, ros_S
      ...
      ! istage>1 and a new function evaluation is needed at the current istage
       ELSEIF ( ros_NewF(istage) ) THEN
         ...
         Ynew(1:N) = MAX(Y(1:N), minkppconc)
         DO j = 1, istage-1
           CALL DAXPY(N,ros_A((istage-1)*(istage-2)/2+j), &
            K(N*(j-1)+1),1,Ynew,1)
         END DO
         CALL FunTemplate(Ynew,Fcn,FIX,RCONST)
         ISTATUS(Nfun) = ISTATUS(Nfun) + 1
...
! MODIFIED BY DP 20180906 to remove T dependence since FunTemplate is not a function of T
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   IMPLICIT NONE

!~~~> Input arguments
   REAL(kind=dp), INTENT(IN) :: T, Roundoff, Y(N), Fcn0(N), FIX(NFIX), RCONST(NREACT)
!~~~> Output arguments
...
!~~~> Local variables
   ...
   CALL FunTemplate(Y,dFdT,FIX,RCONST)
   ISTATUS(Nfun) = ISTATUS(Nfun) + 1
...
!~~~>    Construct Ghimj = 1/(H*gam) - Jac0
     ...
     ghinv = ONE/(Direction*H*gam)
     DO i=1,N
       Ghimj(LU_DIAG_ptr(i)) = Ghimj(LU_DIAG_ptr(i))+ghinv
     END DO
#endif
...
!~~~>    If unsuccessful half the step size; if 5 consecutive fails then return
        ...
        Singular = .TRUE.
        if (chm_lun_out > 0) write(chm_lun_out, *) 'Warning: LU Decomposition returned ISING = ',ISING
...
!~~~> InOut variables
   ...
   IF ( Info < 0 ) THEN
      if (chm_lun_out > 0) write(chm_lun_out, *) "Error in DGETRS. ISING=",ISING
   END IF
#else
   CALL KppSolve_ptr( A, b )
#endif
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 SUBROUTINE FunTemplate( Y, Ydot, FIX, RCONST )
   IMPLICIT NONE
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! Feb 2017 - Jack
!  modified for GEMMACH from: "FunTemplate( T, Y, Ydot )"
!                       to  : "FunTemplate( Y, Ydot, FIX, RCONST )"
!  Now this is 'time independent' as we are not updating reaction rate coeff. within chem.
!   internal timestep. Arrays FIX, RCONST are passed in to avoid OMP>1 thread safety
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Input variables
   ...
   REAL(kind=dp), INTENT(IN)  :: FIX(NFIX), RCONST(NREACT)
!~~~> Output variables
...
!  CALL Update_SUN()
!   RCONST_UPD = RCONST
!   CALL Update_RCONST(RCONST_UPD, Y)
   CALL Fun_ptr( Y, FIX, RCONST, Ydot )
!  TIME = Told

 END SUBROUTINE FunTemplate

!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 SUBROUTINE JacTemplate( Y, Jcb, FIX, RCONST )
   IMPLICIT NONE
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! Feb 2017 - Jack
!  modified for GEMMACH from: "JacTemplate( T, Y, Jcb )"
!                       to  : "JacTemplate( Y, Jcb, FIX, RCONST )"
!  Now this is 'time independent' as we are not updating reaction rate coeff. within chem.
!   internal timestep. Arrays FIX, RCONST are passed in to avoid OMP>1 thread safety
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Input variables
   REAL(kind=dp), INTENT(IN)  :: Y(NVAR), FIX(NFIX), RCONST(NREACT)
!~~~> Output variables
#ifdef FULL_ALGEBRA
   REAL(kind=dp), INTENT(OUT) :: JV(LU_NONZERO), Jcb(NVAR,NVAR)
#else
   REAL(kind=dp), INTENT(OUT) :: Jcb(LU_NONZERO)
#endif
!~~~> Local variables
!   REAL(kind=dp) :: Told
#ifdef FULL_ALGEBRA
...
!   CALL Update_RCONST(RCONST_UPD, Y)
#ifdef FULL_ALGEBRA
   CALL Jac_SP_ptr(Y, FIX, RCONST, JV)
   DO j=1,NVAR
      DO i=1,NVAR
         Jcb(i,j) = 0.0_dp
      END DO
   END DO
   DO i=1,LU_NONZERO
      Jcb(LU_IROW_ptr(i),LU_ICOL_ptr(i)) = JV(i)
   END DO
#else
   CALL Jac_SP_ptr( Y, FIX, RCONST, Jcb )
#endif
!   TIME = Told
