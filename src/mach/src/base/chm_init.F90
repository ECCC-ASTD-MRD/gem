...
!                  this is called once via api/phy_init
!
!==============================================================================
!!if_on
...
!!if_off
   ...
   use chm_nml_mod,          only: chm_master, chm_pkg_gas_s, chm_debug_trace_L
...
!
   if (chm_debug_trace_L) call msg_toall(MSG_INFO, 'chm_init [BEGIN]')
!
   F_istat = -1
...
!
   if (.not. chm_master) then
      ...
      F_istat = 0
      if (chm_debug_trace_L) call msg_toall(MSG_INFO, 'chm_init [EXIT]')
      return
   end if
!
...
!
   F_istat = 1
!
   if (chm_debug_trace_L) call msg_toall(MSG_INFO, 'chm_init [END]')
