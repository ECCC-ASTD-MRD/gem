...
!
!!if_on
subroutine mach_canopy_levels(pvars, metvar3dcan, metvar3dnocan, metvar3d, &
                            metvar2d, imod, imod2, kmod, kcan, ni_can, ni_nocan)
   use phymem,               only: phyvar
   use chm_ptopo_grid_mod,   only: chm_ni, chm_nk, nkt, nkc
   use chm_metvar_mod,       only: SIZE_MV2D, SIZE_MV3D
!!if_off
   use chm_utils_mod,        only: ik, chm_error_l, chm_lun_out, chm_msg_debug
   ...
   use chm_metvar_mod,       only: MV2D_ILMO, MV2D_PPLUS, MV2D_QDIAG, MV2D_UE, &
                                   ...
                                   MV3D_RNFLX, MV3D_SNOFLX, MV3D_LWCRAD
...
! Initializations: Define canopy and non-canopy grid indirect addresses
   ...
   ii = 0
   do i = 1, chm_ni
      if (pvars(sm(sp_FRT)%per_pvarid)%data(i) > 0.0) then
         ic = ic + 1
         imod(ic) = i
      else
         ii = ii + 1
...
! Generate initial canopy levels, as altitude above sea level
      hc(ic) = pvars(sm(sp_HC)%per_pvarid)%data(ii)
!
      do kc = 1, nkc
      ...
      if (local_dbg) then
         do kc = 1, nkc
            if (kcan(ic, kc) < 1) then
               write(chm_lun_out,*) 'mach_canopy_levels: kcan undefined: ', &
                          ic, kc, kcan(ic, kc)
               chm_error_l = .true.
               return
            end if
         ...
         do k = 1, chm_nk
            if (kmod(ic, k) < 1) then
               write(chm_lun_out,*) 'mach_canopy_levels: kmod undefined: ',ic, k, kmod(ic, k)
               chm_error_l = .true.
               return
            end if
         end do
      ...
      do kc = 1, nkc
         do ic = 1, ni_can
            if ((klower_can(ic, kc) /= klower_can(ic, kc)) .or. &
               ...
               write(chm_lun_out,*) 'mach_canopy_levels klower_can after creation NaN or <=0 or >chm_nk+1 : ', &
...
!  Assign those resolved scale values to the canopy layers in the resolved + canopy space (kk):
               metvar3dcan(ic, kk, MV3D_LWCRAD)  = metvar3d(ii, k-1, MV3D_LWCRAD)
...
!  If the canopy layer kc falls below the lowest resolved scale layer...
         if (zcan(ic,kc) < metvar3d(ii, chm_nk, MV3D_ZMOM)) then
            metvar3dcan(ic, kk, MV3D_LWCRAD) = metvar3d(ii, chm_nk, MV3D_LWCRAD)
      ...
      do kk = 1, nkt
         do ic = 1, ni_can
            if (metvar3dcan(ic, kk, MV3D_TPLUS) < 150.0) then
               write(chm_lun_out,*) 'mach_canopy_levels:  suspicious temperature detected in mach_canopy_levels after creation (ic kk value): ',&
                           ic, kk, metvar3dcan(ic, kk, MV3D_TPLUS)
               do kc = 1, nkc
                  write(chm_lun_out,*) 'mach_canopy_levels: value of zcan(ic kc) z2(ic,chm_nk+1) and difference  at this value of ic for kk: ',&
                               kc,' are: ',zcan(ic,kc),z2(ic,chm_nk+1), zcan(ic,kc)-z2(ic,chm_nk+1)
               end do

               do k = 1, nkt
                  write(chm_lun_out,*) 'mach_canopy_levels: value of zthrmcan for ic = ',ic,' at k = ',k,' is: ',zthrmcan(ic,k)
               end do

               do kc = 1, nkc
                  write(chm_lun_out,*) 'mach_canopy_levels:  values of kcan zcan and original zcan for ic = ',ic,' at kc = ',kc,' are: ',&
                              kcan(ic, kc), zcan(ic, kc), hc(ic) * can_frac(kc)
               end do

               do k = 1,chm_nk
                  write(chm_lun_out,*) 'mach_canopy_levels:  value of kmod and z for ic = ',ic,' at k = ',k,' are: ',kmod(ic,k), zt(ic, k)
               end do

               do kc = 1, nkc
                  write(chm_lun_out,*) 'mach_canopy_levels: value of klower_can at this value of ic for kc: ',kc,' is: ',klower_can(ic,kc)
               end do
      ...
      do kc = 1, nkc
         do ic = 1, ni_can
            if (kcan(ic, kc) == 0) then
               write(chm_lun_out,*) 'kcan zero inside mach_canopy_levels at ic kc = ', &
                           ic, kc
               chm_error_l = .true.
               return
            end if
