...
!
   integer(kind=4)  :: ii, istat, jj, busid, idxv1(1), this_ik
...
!==============================================================================
   if (step < 1 .and. var_step > 0) then
      busid = sm(var_step) % per_pvarid
      do jj = 1, chm_nk
         do ii = 1, chm_ni
            this_ik = ik(ii, jj, chm_ni) + 1
            pvars(busid)%data(this_ik) = chm_timestep
         end do
      end do
   end if
!
   ni_can = 0
   if (.not. chm_canopy_shading_l) then
      call msg_toall(chm_msg_debug, 'mach_stepinit [EXIT]')
      return
   end if
!
   if (step < 1) then
...
!    If not using the monthly LAI input;
      if (.not. associated(lai)) then
         lai(1:chm_ni) => pvars(sm(sp_LAI)%per_pvarid)%data(:)
      end if
!
...
!
         beam_penetration = exp(-0.5 * lai(ii) * &
                                pvars(sm(sp_CLU)%per_pvarid)%data(ii))
!
...
! population density decision in the subsequent IF statement.
         pdens = max(pop(ii),1.E-03) / pvars(dxdy)%data(ii)
         if (lai(ii) < laimin .or.      &
             pvars(sm(sp_HC)%per_pvarid)%data(ii) < hcmin .or. &
             pdens > pdensmax .or.      &
             flandnonforest >= noforestcutoff            .or.    &
             ((beam_penetration > beam_min) .and.                &
             (pvars(sm(sp_HC)%per_pvarid)%data(ii) < hcmin2))) then

            pvars(sm(sp_FRT)%per_pvarid)%data(ii) = -1.0
         else
            pvars(sm(sp_FRT)%per_pvarid)%data(ii) = 1.0
            ni_can = ni_can + 1
         end if
...
!
      do ii = 1, chm_ni
         if (pvars(sm(sp_FRT)%per_pvarid)%data(ii) > 0.0) then
            ni_can = ni_can + 1
         end if
      end do
!
