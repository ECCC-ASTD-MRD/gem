...
!
!!if_on
subroutine mach_mie_opt(pvars, aero_tr, trwtrow, thlev, rhoa, pni, pnk,&
                        nmod, kmod)
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_mie_opt [BEGIN]')
   ...
   do nn = 1, nwl_aod
      ...
      do kz = nk_start_pm, chm_nk
         do ii = 1, pni
            ...
            ix = nmod(ii)
            this_ik = ik(ix, kz, chm_ni) + 1
            pvars(species_master(opt_id)%per_pvarid)%data(this_ik) = totopt(ii, kk, nn)
            pvars(species_master(ssc_id)%per_pvarid)%data(this_ik) = sscalb(ii, kk, nn)
            pvars(species_master(asy_id)%per_pvarid)%data(this_ik) = asymbn(ii, kk, nn)
         end do
      end do
!
! Save column (2-D) total aerosol optical depth to output:
      if (species_master(opt_id)%out_pvarid > 0) then
         do ii = 1, pni
            ix = nmod(ii)
            zoptd(ii, nn) = 0.0
            do kk = 1, pnk
            ...
            pvars(species_master(opt_id)%out_pvarid)%data(ix) = zoptd(ii, nn)
         end do
      end if
   end do

   call msg_toall(chm_msg_debug, 'mach_mie_opt [END]')
