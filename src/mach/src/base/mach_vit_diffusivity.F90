...
!  Debug tracing
   call msg_toall(chm_msg_debug, 'mach_vit_diffusivity [BEGIN]')
!
...
!  number of vehicles going past a single point in the grid cell.
   do ii = 1, ni
      ...
      Fc(ii) = pvars(sm(sp_CVKT)%per_pvarid)%data(im) * odx
      Fm(ii) = pvars(sm(sp_MVKT)%per_pvarid)%data(im) * odx
      Ft(ii) = pvars(sm(sp_TVKT)%per_pvarid)%data(im) * odx
   end do
!
!-------
...
!
   call msg_toall(chm_msg_debug, 'mach_vit_diffusivity [END]')
!
   return
