...
!
!!if_on
subroutine mach_pm_chem(pvars, chem_tr, metvar2d, pmetvar3d, fland, &
...
! 2D fields
   do ix = 1, pni
      ...
      twater(ix) = metvar2d(ii, MV2D_TWATER)
!
      do l = 1, lucprm
         this_ik = ik(ii, l, chm_ni) + 1
         pfland(ix, l) = fland(ii, l)
         ra(ix, l) = pvars(sm(sp_LU15)%ra_pvarid)%data(this_ik)
      end do
   end do
   ...
   if (chm_soa_s /= 'NIL')  then
      do kz = nk_start_pm, chm_nk
         do ix = 1, pni
            ...
            this_ik = ik(nmod(ix), kz, chm_ni) + 1
            soa(ix, kk) = pvars(sm(sp_NWOC)%out_pvarid)%data(this_ik) * &
      ...
      if (canopy_columns) then
         do kc = 1, nkc
            do ix = 1, pni
               ...
               this_ik = ik(nmod(ix), kc, chm_ni) + 1
               soa(ix, kk) = pvars(sm(sp_NWOC)%vd_pvarid)%data(this_ik) * &
                             ug2kg / chm_timestep !ug/kg->kg/kg/s
            end do
         end do
      end if
...
!
!  load needed gas and aerosol tracers (!ug/kg -> kg/kg)
   do kz = nk_start_pm, chm_nk
      do ix = 1, pni
         k2 = kmod(ix, kz)
         kk = k2 - nk_start_pm + 1
...
!  sea-salt surface flux
         call mach_cam_sfss(tdiag, surfwd, rsfrow, pfland, twater, pni)
...
! Wet deposition diagnostic - HARDCODE negative
   if (chm_diag_wetdep_L) then
      do ix = 1, pni
         ii = nmod(ix)
         pvars(sm(sp_WSO3)%wd_pvarid)%data(ii) = -wetflx(ix, 1) + &
                               pvars(sm(sp_WSO3)%wd_pvarid)%data(ii)
         pvars(sm(sp_H2O2)%wd_pvarid)%data(ii) = -wetflx(ix, 2) + &
                               pvars(sm(sp_H2O2)%wd_pvarid)%data(ii)
         pvars(sm(sp_gOOH)%wd_pvarid)%data(ii) = -wetflx(ix, 3) + &
                               pvars(sm(sp_gOOH)%wd_pvarid)%data(ii)
         pvars(sm(sp_CAT) %wd_pvarid)%data(ii) = -wetflx(ix, 7) + &
                               pvars(sm(sp_CAT) %wd_pvarid)%data(ii)
         pvars(sm(sp_HCO3)%wd_pvarid)%data(ii) = -wetflx(ix, 8) + &
                               pvars(sm(sp_HCO3)%wd_pvarid)%data(ii)
         pvars(sm(sp_HION)%wd_pvarid)%data(ii) = -wetflx(ix, 9) + &
                               pvars(sm(sp_HION)%wd_pvarid)%data(ii)
         pvars(sm(sp_WH2O)%wd_pvarid)%data(ii) = -wetflx(ix, 10) + &
                               pvars(sm(sp_WH2O)%wd_pvarid)%data(ii)
      end do
      do iae = 1, icom
         nsp = iae * isize + sp_AERO - 1  ! Use the species last bin's index
         if (sm(nsp)%wd_pvarid > 0) then
            iwf = ip_wflx(iae)               ! wet dep index
            do ix = 1, pni
               ii = nmod(ix)
               pvars(sm(nsp)%wd_pvarid)%data(ii) = - wetflx(ix, iwf) + &
                                     pvars(sm(nsp)%wd_pvarid)%data(ii)
            end do
         end if
      end do
   end if
...
! Dry deposition diagnostic for aerosols
   do iae = 1, icom
      ...
      if (sm(nsp)%dd_pvarid > 0) then
         do ix = 1, pni
            ii = nmod(ix)
            pvars(sm(nsp)%dd_pvarid)%data(ii) = pvars(sm(nsp)%dd_pvarid)%data(ii) + &
                                                   dryflx(ix, iae)
         end do
      end if
   end do
...
! Diagnostic aerosol optical properties (for estimating AOD, etc)
   if (chm_diag_aero_opt_l) then
      call mach_mie_opt(pvars, aero_tr, trwtrow, thlev, rho, pni, pnk, &
                        nmod, kmod)
   end if
