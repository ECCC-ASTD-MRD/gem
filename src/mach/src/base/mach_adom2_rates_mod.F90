...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_adom2_jcorr [BEGIN]')
...
!  calculate the final clear sky correction factor:
   ...
   call msg_toall(chm_msg_debug, 'mach_adom2_jcorr [END]')
...
!============================================================================
  subroutine mach_gas_lookup_jvals(zmount, skyo, csza, rg, gni, gnk, npt)
   use chm_consphychm_mod, only: rad2deg
   implicit none
!
...
!
   call msg_toall(chm_msg_debug, 'mach_gas_lookup_jvals [BEGIN]')
!
!------------------------------------------------------------------
...
!
   ...
   do k = nk_start, gnk
      do i = 1, gni
         sky(jpt) = skyo(i, k)
         zen(jpt) = acos(csza(i)) * rad2deg ! convert radians to degrees
...
!-----------------------------------------------------------------------
   ...
   call msg_toall(chm_msg_debug, 'mach_gas_lookup_jvals [END]')
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_adom2_bcs_coeffs [BEGIN]')
...
! --- aromatics
   ...
   call msg_toall(chm_msg_debug, 'mach_adom2_bcs_coeffs [END]')
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_adom2_uprate [BEGIN]')
...
!
   call msg_toall(chm_msg_debug, 'mach_adom2_uprate [END]')
!
   return
...
!
   call msg_toall(MSG_DEBUG, 'mach_adom2_chemi [BEGIN]')
!
...
! --- alkanes
   ...
   call msg_toall(MSG_DEBUG, 'mach_adom2_chemi [END]')
