...
! Stuff that comes from the permanent bus
   ...
   integer(kind=4), parameter :: MV2D_TWATER   =18  ! Sea-surface temperature

! Stuff that comes from the volatile bus
   integer(kind=4), parameter :: MV2D_UE       = 19 ! Friction velocity u* from surface momentum flux
   integer(kind=4), parameter :: MV2D_CANG     = 20 ! Cosine of solar zenith
   integer(kind=4), parameter :: MV2D_RAINRATE = 21 ! liquid precip. rate
   integer(kind=4), parameter :: MV2D_SNOF     = 22 ! Snow fraction
   integer(kind=4), parameter :: MV2D_WDDIAG   = 23 ! Screen level wind direction

   integer(kind=4), parameter :: SIZE_MV2D     = 23
!
! 3D array
...
! Used in MESSY rad. transfer
   ...
   integer(kind=4), parameter :: MV3D_LWCRAD   = 18 ! specific liquid water content for radiation
