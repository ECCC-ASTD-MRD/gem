...
!!if_on
   ...
   real(kind=4),    intent   (in) :: twater(pni)
!!if_off
!
! Local variables
!
...
!  NOTE: Sea-ice presence accounted for already in fland in mach_landuse
            if (fland(i, 14) > 0.0) then
               if (chm_seaflux_s(1:12) == 'GONG_MONAHAN') then  !gong's scheme
                  if (chm_seaflux_sst_l) then
                     df0(i) = df0(i) + delr0 * pmass * mod_sst(twater(i)) * flx0(r1(i) + delr0 * real(pos - 1))
                  else
...
!  Jaelé et all 2011 SST modulation factor
   real(kind=4) function mod_sst(T)
      implicit none
      real(kind=4), intent(in) :: T ! Sea-surface temperature
      
      mod_sst = 0.3 + 0.1 * T - 0.0076 * T * T + 0.00021 * T * T * T
      return
   end function mod_sst
