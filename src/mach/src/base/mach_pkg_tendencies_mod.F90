...
module mach_pkg_tendencies_mod
   ...
   use chm_utils_mod,        only: chm_lun_out, NOMV_LEN, LONG_VARNAME, &
                                   chm_msg_debug
...
!        PM
   ...
   subroutine tendency_store(process_id, chem_tr, pvars)
...
!! @param INOUT pvars phybus pointer
!============================================================================
      ...
      local_dbg = (.false. .and. (chm_lun_out > 0))
      call msg_toall(chm_msg_debug, 'tendency_store [BEGIN]')
            ...
            do sp = 1, nb_dyn_tracers
               do k = 1, chm_nk
                  do i = 1, chm_ni
                     ...
                     pvars(sm(sp)%td_pvarid)%data(this_ik) = chem_tr(i, k, sp)
                  end do
               end do
            end do
         case (TEND_GASCHEM)
            do sp = 1, nb_dyn_tracers
               do k = 1, chm_nk
                  do i = 1, chm_ni
                     this_ik = ik(i, k, chm_ni) + 1
                     pvars(sm(sp)%tg_pvarid)%data(this_ik) = chem_tr(i, k, sp)
                  end do
               end do
            end do
         case (TEND_PMCHEM)
            do sp = 1, nb_dyn_tracers
               do k = 1, chm_nk
                  do i = 1, chm_ni
                     this_ik = ik(i, k, chm_ni) + 1
                     pvars(sm(sp)%tp_pvarid)%data(this_ik) = chem_tr(i, k, sp)
                  end do
               end do
            end do
         case default
      ...
      call msg_toall(chm_msg_debug, 'tendency_store [END]')

   end subroutine tendency_store


   subroutine tendency_delta(process_id, chem_tr, pvars)
!============================================================================
!! Description    : Store the value of all species' concentrations in
!!                  order to diff it after the process' completion.
...
!============================================================================
      integer(kind=4), intent(in)    :: process_id
      real(kind=4)   , intent(in)    :: chem_tr(chm_ni, chm_nk+1, nb_dyn_tracers)
      real(kind=4)   , dimension(:), pointer, contiguous :: pvars

      integer(kind=4) i, k, this_ik
      logical(kind=4) local_dbg

      local_dbg = (.false. .and. (chm_lun_out > 0))
      call msg_toall(chm_msg_debug, 'tendency_delta [BEGIN]')
            ...
            do sp = 1, nb_dyn_tracers
               do k = 1, chm_nk
                  do i = 1, chm_ni
                     this_ik = ik(i, k, chm_ni) + 1
                     pvars(sm(sp)%td_pvarid)%data(this_ik) = chem_tr(i, k, sp) - &
                                              pvars(sm(sp)%td_pvarid)%data(this_ik)
                  end do
               end do
            end do
         case (TEND_GASCHEM)
            do sp = 1, nb_dyn_tracers
               do k = 1, chm_nk
                  do i = 1, chm_ni
                     this_ik = ik(i, k, chm_ni) + 1
                     pvars(sm(sp)%tg_pvarid)%data(this_ik) = chem_tr(i, k, sp) - &
                                              pvars(sm(sp)%tg_pvarid)%data(this_ik)
                  end do
               end do
            end do
         case (TEND_PMCHEM)
            do sp = 1, nb_dyn_tracers
               do k = 1, chm_nk
                  do i = 1, chm_ni
                     this_ik = ik(i, k, chm_ni) + 1
                     pvars(sm(sp)%tp_pvarid)%data(this_ik) = chem_tr(i, k, sp) - &
                                              pvars(sm(sp)%tp_pvarid)%data(this_ik)
                  end do
               end do
            end do
         case default
      ...
      call msg_toall(chm_msg_debug, 'tendency_delta [END]')
