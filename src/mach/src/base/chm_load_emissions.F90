...
! Description    : Read from file for major point sources
!                  called per step via rpnphy api/phy_input.F90
...
!!if_off
   ...
   use chm_nml_mod,          only: chm_master, chm_emis_master, chm_get_mj_emis_l, &
                                   ...
                                   chm_step_factor, chm_cffeps_online_l, chm_debug_trace_L
...
!
   ...
   integer(kind=4)                              :: datev, ier, err, istatus, iverb
   integer(kind=4)                              :: int_x, int_y
...
!  Declaration of external functions and subroutines
   ...
   call msg_toall(chm_msg_debug, 'chm_load_emissions [BEGIN]')
! Set debug switch
...
!
 ...
 1001 format(/,'CHEMISTRY : PERFORMING TIMESTEP #',I9, &
             /,'========================================')
   call msg_toall(chm_msg_debug, 'chm_load_emissions [END]')
   call msg_verbosity(iverb)
