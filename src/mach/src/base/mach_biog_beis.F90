...
!!if_off
   ...
   use chm_utils_mod,        only: chm_error_l, chm_lun_out, chm_msg_debug
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_biog_beis [BEGIN]')
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_biog_beis [END]')
   return
!
   contains
!
...
!  of original model levels!
            ...
            do kk = nkc, 1, -1
               ...
               write(chm_lun_out,*) 'Layer mismatch in biogenic level to canopy level matching - stopping code'
               write(chm_lun_out,*) 'sum of non-zero contributions from 3 canopy layer in column ',ic, &
               ' layer ',kk,' is ',sumc2(kk),' (should be unity)'
               write(chm_lun_out,*) 'values of zmombio: ',(zmombio(k), k = 1,nkc+1)
               write(chm_lun_out,*) 'values of zmomcan: '
               do k = kcan_top, nkt+1
                  write(*,*) k, zmom_1d(k)
               end do
               write(chm_lun_out,*) 'values of nfrcbio: ',(nfrcbio(k,ic), k = 1,nkt)
               write(chm_lun_out,*) 'values of ifrcbio: ',((ifrcbio(k2,k,ic), k2 = 1, nfrcbio(k,ic)), k = 1,nkt)
               write(chm_lun_out,*) 'values of frcbio: ',((frcbio(k2,k,ic), k2 = 1, nfrcbio(k,ic)), k = 1,nkt)
               write(chm_lun_out,*) 'values of sumc2: ',(sumc2(k), k=1,nkc)
               chm_error_l = .true.
               return
               end if
            end do
