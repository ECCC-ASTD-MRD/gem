...
!
!!if_on
subroutine mach_calc_diag(pvars, chem_tr, metvar2d, metvar3d, step)
...
!  Debugging
   ...
   call msg_toall(chm_msg_debug, 'mach_calc_diag [BEGIN]')
!
...
! Overhead column water vapor
   call ghg_xcol1(hu_vmr, p_bot, p_top, hu_over, chm_ni, chm_nk)
!
! Overhead column of O3 NO2 NO PAN HNO3 NO3 N2O5 SO2 SO4 CO HCHO ISOP
   do isp = 1, nspecies
      ...
      call ghg_xcol1(var_vmr, p_bot, p_top, var_over, chm_ni, chm_nk)
...
!  YHU: Column water vapor
            pvars(sm(sp_col_hu)%out_pvarid)%data(this_ik) = hu_over(i, k)
!  Column O3, NO2, and NO 1E+15 (Peta) molecules cm-2 (YO3, YNO2, YNO);
            do isp = 1, 3
               sp_indx = sp_col(isp)
               pvars(sm(sp_indx)%out_pvarid)%data(this_ik) = sp_over(i, k, isp)
               pvars(sm(sp_indx)%out_pvarid)%data(this_ik) = sp_over(i, k, isp)
               pvars(sm(sp_indx)%out_pvarid)%data(this_ik) = sp_over(i, k, isp)
            end do
         end do
...
! ZO3 ZNO2 ZNOX ZPAN ZHN3 ZN25 ZSO2 ZSO4 ZCO ZHCH ZISO
      do isp = 1, nspecies
         sp_indx = sp_tcol(isp)
         do i = 1, chm_ni
            pvars(sm(sp_indx)%out_pvarid)%data(i) = sp_over(i, chm_nk, isp)
         end do
      end do
!
   end if
...
!
         do isp = 1, nspecies
            do i = 1, chm_ni
               pvars(sm(sp_tcol(isp))%per_pvarid)%data(i) = 0.0
            end do
         end do
!
         do k = 1, chm_nk
            do i = 1, chm_ni
               this_ik = ik(i, k, chm_ni) + 1
!
               pvars(sm(sp_col_O3) %per_pvarid)%data(this_ik) = 0.0
               pvars(sm(sp_col_NO2)%per_pvarid)%data(this_ik) = 0.0
               pvars(sm(sp_col_NO) %per_pvarid)%data(this_ik) = 0.0
               pvars(sm(sp_col_HU) %per_pvarid)%data(this_ik) = 0.0
               pvars(sm(sp_tend_O3)%per_pvarid)%data(this_ik) = 0.0
!
               do isp = 1, nspecies
                  pvars(sm(sp_conc(isp))%per_pvarid)%data(this_ik) = 0.0
               end do
            end do
         end do
!
...
!
      do isp = 1, nspecies
         sp_indx = sp_tcol(isp)
         do i = 1, chm_ni
            pvars(sm(sp_indx)%per_pvarid)%data(i) = sp_over(i, chm_nk, isp) + &
                                  pvars(sm(sp_indx)%per_pvarid)%data(i)
         end do
      end do
!
      if (mod(step, int(moyhr_steps)) == 0) then
         do isp = 1, nspecies
            ...
            do i = 1, chm_ni
               pvars(sm(sp_indx)%per_pvarid)%data(i) = &
                         pvars(sm(sp_indx)%per_pvarid)%data(i) * moysti
            end do
         end do
      end if
!
...
! AYHU (Peta molec. cm-2)
            pvars(sm(sp_col_HU)%per_pvarid)%data(this_ik) = hu_over(i, k) + &
                      pvars(sm(sp_col_HU)%per_pvarid)%data(this_ik)
!
! AYO3 (D.U.); AYN2 & AYNO (Peta molec. cm-2)
            do isp = 1, 3
               pvars(sm(sp_col(isp))%per_pvarid)%data(this_ik) = &
                 sp_over(i, k, isp) + pvars(sm(sp_col(isp))%per_pvarid)%data(this_ik)
            end do
!
! AGOZ  (ug /kg /sec)
            pvars(sm(sp_tend_O3)%per_pvarid)%data(this_ik) = &
                      pvars(sm(sp_tend_O3)%per_pvarid)%data(this_ik) + &
                      pvars(sm(sp_tend_O3)%out_pvarid)%data(this_ik)
!
! AMO3 AMN2 AMNO AMPN AMH3 AM25 AMS2 AMS4 AMCO AMHC AMIS  (ug /kg)
            do isp = 1, nspecies
               sp_indx = sp_conc(isp)
               pvars(sm(sp_indx)%per_pvarid)%data(this_ik) = &
                            pvars(sm(sp_indx)%per_pvarid)%data(this_ik) + &
                            chem_tr(i, k, sp_indx)
            end do
...
!
               pvars(sm(sp_tend_O3)%per_pvarid)%data(this_ik) = &
                         pvars(sm(sp_tend_O3)%per_pvarid)%data(this_ik) * moysti
               do isp = 1, nspecies
                  sp_indx = sp_conc(isp)
                  pvars(sm(sp_indx)%per_pvarid)%data(this_ik) = &
                         pvars(sm(sp_indx)%per_pvarid)%data(this_ik) * moysti
               end do
!
            end do  !i = 1, chm_ni
         end do    !k = 1, chm_nk
...
!         pvars(sm(sp_tcol_O3)%per_pvarid)%data(i)  = 0.0
!      end do
...
!         pvars(sm(sp_tcol_O3 )%per_pvarid)%data(i) = &
!                   pvars(sm(sp_tcol_O3 )%per_pvarid)%data(i)  + &
!                   pvars(sm(sp_tcol_O3 )% out_pvarid)%data(i) * chm_timestep
...
!            pvars(sm(sp_tend_O3) %per_pvarid)%data(this_ik) = &
!                   pvars(sm(sp_tend_O3  )%per_pvarid)%data(this_ik) + &
!                   pvars(sm(sp_tend_O3  )%out_pvarid)%data(this_ik) * chm_timestep
!         end do
!      end do
!
!   end if
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_calc_diag [END]')
