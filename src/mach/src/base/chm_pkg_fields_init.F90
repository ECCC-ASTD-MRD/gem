...
!!if_off
   use chm_nml_mod,             only: chm_model_s      , chm_pkg_gas_s   ,  &
                                      ...
                                      chm_diag_aerosols_l
   use chm_utils_mod,           only: chm_lun_out, MAX_DEBUG_VAR, chm_msg_debug
...
!
   ...
   call msg_toall(chm_msg_debug, 'chm_pkg_fields_init [BEGIN]')
...
!     Gas package
         if (chm_pkg_gas_s(1:5) == 'ADOM2') then
            ...
            write(chm_lun_out, *) '### Error in chm_pkg_fields_init ###'
            write(chm_lun_out, *) '# GAS fields package unknown: ', chm_pkg_gas_s
...
!    Particule package
         select case (chm_pkg_pm_s)
            ...
            case default
               write(chm_lun_out, *) '### Error in chm_pkg_fields_init ###'
               write(chm_lun_out, *) '# PM fields package unknown: ', chm_pkg_pm_s
               write(chm_lun_out, *) '###         ABORT         ###'
               chm_pkg_fields_init = .false.
               return
         end select
...
!
      case default
         write(chm_lun_out, *) '###         ABORT                           ###'
         write(chm_lun_out, *) '###         CHEMICAL PACKAGE UNKNOWN        ###'
         chm_pkg_fields_init = .false.
         return
   end select
...
!
   if ((chm_debug_2d_i >= 0) .or. (chm_debug_3d_i >= 0 )) then
      if (chm_debug_2d_i > MAX_DEBUG_VAR .or. chm_debug_3d_i > MAX_DEBUG_VAR) then
         write(chm_lun_out, *) '### Error in chm_pkg_fields_init ###'
         write(chm_lun_out, *) '# A maximum of ', MAX_DEBUG_VAR, ' debug '
         write(chm_lun_out, *) '# variables is allowed for 2D and 3D (32 total)'
         write(chm_lun_out, *) '# You asked for ', chm_debug_2d_i, ' 2D and ', chm_debug_3d_i, ' 3D'
         write(chm_lun_out, *) '###         ABORT         ###'
         chm_pkg_fields_init = .false.
         return
      end if
      tmp = pkg_debug_idxinit(nb_fields, chm_debug_2d_i, chm_debug_3d_i)
       ...
       write(chm_lun_out, *) '### Error in chm_pkg_fields_init ###'
       write(chm_lun_out, *) '# You asked for a negative number of debug variables'
       write(chm_lun_out, *) '# chm_debug_2d_i = ', chm_debug_2d_i, ', chm_debug_3d_i ', chm_debug_3d_i
       write(chm_lun_out, *) '###         ABORT         ###'
       chm_pkg_fields_init = .false.
       return
   end if
...
!
   ...
   call msg_toall(chm_msg_debug, 'chm_pkg_fields_init [END]')
