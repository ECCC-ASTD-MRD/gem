...
!!if_off
   use chm_nml_mod,   only: dbg_lat, dbg_lon, chm_debug_trace_L
...
!  Declaration of external functions and subroutines
   ...
   if (chm_debug_trace_L) call msg_toall(MSG_INFO, 'chm_set_dbg_point [BEGIN]')

   if (abs(dbg_lat) > 90.0) then
      if (chm_lun_out > 0) &
         write(chm_lun_out, *) "CHM debug coordinate not set"
      chm_set_dbg_point = 1
      if (chm_debug_trace_L) call msg_toall(MSG_INFO, 'chm_set_dbg_point [EXIT]')
      return
   end if
...
!                        int_y - ly_lbound + 1
      ...
      if (dbg_loc(3) < 0) then
         write(chm_lun_out, *) 'WARNING !!! The requested GEM-MACH debug locations ', &
                     dbg_lon, ' and ', dbg_lat, ' are outside the model domain '
      else
         write(chm_lun_out, *) 'GEM-MACH debug co-ordinate (dbg_id, dbg_jd) at (',   &
...
!***  Assign the debug coordinates id, and jd in the appropriate PE
   ...
   if (my_pe == dbg_loc(3)) then
      ...
      dbg_id = ij(1)
      dbg_jd = ij(2)
      write(chm_lun_out, *) 'debug location on this PE ', dbg_id, dbg_jd, int_x, int_y
      write(chm_lun_out, *) 'debug location on entire model grid ', dbg_loc(1), dbg_loc(2)
   else
      dbg_id = -1
      dbg_jd = -1
   end if
   ...
   if (chm_debug_trace_L) call msg_toall(MSG_INFO, 'chm_set_dbg_point [END]')
