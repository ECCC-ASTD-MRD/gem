...
!
!!if_on
subroutine mach_diffusion(pvars, conc, vd, psurf, dxdy, rho, kt, vt, &
...
!!if_off
   use chm_utils_mod,        only: chm_timestep, chm_lun_out, chm_msg_debug
...
!
   ...
   local_dbg_2d = (.false. .and. chm_debug_2d_i > 3)
...
!  build mass before in dbg_2d(1)
   if (local_dbg_2d .and. (dbg_itr > 0)) then
      ...
      do i = 1, dni
         ...
         pvars(sm(dbg_2d(1))%out_pvarid)%data(nmod(i)) = mass_sum1
...
!     Meteorological modulation of aerosol emissions
      ...
      if (chm_met_modulation_s /= 'NIL') then
         do i = 1, dni
            fmet(i) = pvars(sm(sp_FMET)%out_pvarid)%data(nmod(i))
         end do
      end if
         ...
         if (sm(sp)%ae_pvarid > 0) then
            do i = 1, dni
               emissions(i, sp) = emissions(i, sp) + &
                    (pvars(sm(sp)%ae_pvarid)%data(nmod(i)) * g2ug * fmet_sp(i))
            end do
         end if
...
!  original units are g/s:  convert to ug/s
         if (sm(sp)%fae_pvarid > 0) then
            do i = 1, dni
               emissions(i, sp) = emissions(i, sp) + &
                    (pvars(sm(sp)%fae_pvarid)%data(nmod(i)) * g2ug * fmet(i))
            end do
         end if
!
...
!  original units are g/s:  convert to ug/s
         if (sm(sp)%be_pvarid > 0) then
            do i = 1, dni
               emissions(i, sp) = emissions(i, sp) + &
                                  (pvars(sm(sp)%be_pvarid)%data(nmod(i)) * g2ug)
            end do
         end if
!
!  Bidirectional flux emissions added to emissions array:
!  original units are g/s:  convert to ug/s
         if (sm(sp)%bd_pvarid > 0) then
            do i = 1, dni
               emissions(i, sp) = emissions(i, sp) + &
                                  (pvars(sm(sp)%bd_pvarid)%data(nmod(i)) * g2ug)
            end do
         end if
      end do
   ...
   if (echoice > 1) then
      do sp = 1, nb_dyn_tracers
         if (sm(sp)%mae_pvarid > 0) then
            do i = 1, dni
               emissions(i, sp) = emissions(i, sp) + &
                      pvars(sm(sp)%mae_pvarid)%data(nmod(i)) * g2ug
            end do
         end if
      end do
...
!  build mass of emissions in dbg_2d(4)
   if (local_dbg_2d .and. (dbg_itr > 0)) then
      ...
      do i = 1, dni
         ...
         pvars(sm(dbg_2d(2))%out_pvarid)%data(nmod(i)) = mass_sum1
         pvars(sm(dbg_2d(3))%out_pvarid)%data(nmod(i)) = mass_sum2
         pvars(sm(dbg_2d(4))%out_pvarid)%data(nmod(i)) = chm_timestep * emissions(i, spx)
