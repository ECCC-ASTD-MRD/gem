...
!
!!if_on
subroutine mach_main(pvars, chem_tr, metvar2d, metvar3d, slab_index, &
...
!!if_off
   use chm_utils_mod,        only: chm_lun_out, chm_error_l, ik, chm_msg_debug
...
!
   ...
   write(tmp_S, '(a,i5,a,i3)')'slab_index:',slab_index,' step:', step
   call msg_toall(chm_msg_debug, trim(tmp_S) // 'mach_main [BEGIN]')
!
!================================================================================
! Start of landuse and season calculations
!================================================================================
!
   call mach_landuse(pvars, metvar2d, landuse)
   if (chm_error_l) return
!
   if (step <  1) then
      if (chm_sat_seasons_l) then
         call mach_lai_adjust(pvars, landuse, slab_index)
         if (chm_error_l) return
      end if
!
      call mach_output(pvars, chem_tr, metvar2d, metvar3d, landuse, slab_index)
!
      call msg_toall(chm_msg_debug, 'Initialization for timestep 0 completed. Exiting mach_main.')
      return
   end if
!
...
!
      case ('ROBICHAUD', 'ROBICHAUD2', 'ROBICHAUD3')
         ...
         call mach_gas_drydep_main(pvars, chem_tr, metvar2d, metvar3d, &
                                   landuse, iseasn)
      case default
...
!
         case ('PLUMERISE', 'PLUMERISE2')
            ...
            call mach_plumerise(pvars, chem_tr, metvar2d, metvar3d, landuse, &
                                slab_index)
            if (chm_error_l) return
!
         case default
...
!
   if (ni_can > 0) then
      call mach_canopy_levels(pvars, metvar3dcan, metvar3dnocan, metvar3d, &
...
!
   if (trim(chm_biog_s) /= 'NIL') then
      ...
      call mach_biog_main(pvars, metvar2d, metvar3d, iseasn, &
...
!
   if (chm_pkg_pm_s(1:3) == 'CAM') then
      call mach_cam_flux(pvars, metvar2d, landuse)
   end if
!
!===============================================================================
...
!
   if (chm_vert_diff_s /= 'NIL') then
      ...
      call mach_input_check (pvars, metvar2d, metvar3d, landuse)
!
#if defined(MACH_TENDENCIES)
      call tendency_store(tend_diffusion, chem_tr, pvars)
#endif
!
! Calling diffusion for all species
      call mach_diff_prep(pvars, chem_tr, metvar2d, metvar3d, &
                          metvar3dnocan, metvar3dcan, emisbio_can, tracers_can, &
                          kmod, kcan, imod, imod2, ni_can, ni_nocan)
!
#if defined(MACH_TENDENCIES)
      call tendency_delta(tend_diffusion, chem_tr, pvars)
#endif
   else
      write (chm_lun_out, *) '> Warning: No mach_diffusion'
   end if
!
!===============================================================================
...
#if defined(MACH_TENDENCIES)
      call tendency_store(tend_gaschem, chem_tr, pvars)
#endif
!
      if (ni_nocan > 0) then
         call mach_gas_main(pvars, chem_tr, metvar2d, metvar3dnocan, &
              step, ni_nocan, imod2, landuse)
         if (chm_error_l) return
      end if
      if (ni_can > 0) then
         call mach_gas_canopy(pvars, chem_tr, tracers_can, metvar2d, &
              metvar3dcan, step, ni_can, imod, kcan, kmod, landuse)
         if (chm_error_l) return
      end if
#if defined(MACH_TENDENCIES)
      call tendency_delta(tend_gaschem, chem_tr, pvars)
#endif
!
   else
      write (chm_lun_out, *) '> Warning '
...
#if defined(MACH_TENDENCIES)
          call tendency_store(tend_pmchem, chem_tr, pvars)
#endif
!  Columns without vegetation canopies
          if (ni_nocan > 0) then
             call mach_pm_chem(pvars, chem_tr, metvar2d, metvar3dnocan,&
...
!  Columns with vegetation canopies
          if (ni_can > 0) then
             call mach_pm_chem(pvars, chem_tr, metvar2d, metvar3dcan, &
                  landuse, oldso4_can, iseasn, step, slab_index, ni_can, &
                  pm_nkc, nkt, imod, kmod, tracers_can, kcan)
             if (chm_error_l) return
          end if
#if defined(MACH_TENDENCIES)
          call tendency_delta(tend_pmchem, chem_tr, pvars)
#endif
!
       case ('GOCART_SO2')
          if (local_dbg) then
...
!
      call mach_calc_diag(pvars, chem_tr, metvar2d, metvar3d, step)
   end if
!
!===============================================================================
