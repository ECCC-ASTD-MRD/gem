...
!!if_off
   use chm_consphychm_mod,      only: tcdk
   use chm_utils_mod,           only: chm_lun_out, chm_error_l, &
                                      dbg_id, dbg_jd, chm_msg_debug
   ...
   use chm_nml_mod,             only: chm_hetchem_s, chm_aqueous_s, chm_soa_s, &
                                      ...
                                      dbg_lev, dbg_step, dbg_itr, chm_fctr_adj_s
   implicit none
!!if_on
...
!
   ...
   real(kind=4), dimension(pni, pnk)             :: adj_ctr ! rainout adjustment
   real(kind=4), dimension(pni, pnk, ntr)        :: rtcoa, rtbcld
   logical(kind=4)                               :: local_dbg
   real(kind=4)                                  :: tempc, fice
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_cam_main [BEGIN]')
...
!  RTSO2 is not used in mach_cam_aerocld, WG mar 2001.
   ...
   if (chm_aqueous_s(1:4) == 'GONG' .or. chm_hetchem_s /= 'NIL') then

! rainout test for consideration of the WBF (Wegener-Bergeron-Findeisen) process
! in mixed pahse clouds: adjustment factor to account for possible reduction in
! aqueous-phase tracers being carried over to precipitation due to droplet
! evaporation in presence of ice via either a temperature dependency or ice
! fraction dependency based on Verheggen et al. (2007):
!  1. via temperature dependency (for temp < -5 degC)
!    Fn(Dp>100nm) = 0.031 + 0.93/(1. + exp(-(tempC+3.64)/3.42))
!           for tempC < 0 degC, based on measurements at Jungfraujoch
!     The adjustment to fctr is based on the ratio of Fn at given tempC to
! Fn at tempC = -5:
!    Fn/Fn_-5C = (0.031 + 0.93/(1. + exp(-(tempC+3.64)/3.42)))/ &
!                (0.031 + 0.93/(1. + exp(1.36/3.42)))
!  1a. via temperature dependency (for temp < 0 degC)
!     The adjustment to fctr is based on the ratio of Fn at given tempC to
! Fn at tempC = 0:
!    Fn/Fn_-5C = (0.031 + 0.93/(1. + exp(-(tempC+3.64)/3.42)))/ &
!                (0.031 + 0.93/(1. + exp(-3.64/3.42)))
!  2. via ice mass fraction (IMF)
!    Fn(Dp_100nm) = 0.051 + 0.84exp(-8.55sqrt(IMF))
!     The adjustment to fctr is then based on the ratio of Fn at given IMF
! to Fn at IMF=0:
!    Fn/Fn(IMF=0) = (0.051 + 0.84exp(-8.55sqrt(IMF)))/(0.051 + 0.84)
!
     adj_ctr = 1.0
     select case(trim(chm_fctr_adj_s))
        case ('TEMP')
           do l = 1, pnk
             do i = 1, pni
              tempc = throw(i,l) - tcdk
              if(tempc < -5.) adj_ctr(i,l) = (0.031 + 0.93/ &
                   (1. + exp(-(tempc+3.64)/3.42))) * 2.4707
             end do
            end do
         case ('TEMP1')
            do l = 1, pnk
             do i = 1, pni
              tempc = throw(i,l) - tcdk
              if(tempc < 0.) adj_ctr(i,l) = (0.031 + 0.93/ &
                   (1. + exp(-(tempc+3.64)/3.42))) * 1.3841
             end do
            end do
         case ('IMF')
            do l = 1, pnk
             do i = 1, pni
              fice = min(1., max(0., qr(i,l,2)/(qr(i,l,1) + qr(i,l,2) + smf)))
              adj_ctr(i,l) = (0.051 + 0.84 * exp(-8.55 * fice ** 0.5)) &
                                     * 1.1223
             end do
            end do
         case ('NIL')
            adj_ctr = 1.0
         case default
            write(chm_lun_out, *)' Error in chm_fctr_adj_s option:', chm_fctr_adj_s
            chm_error_l = .true.
            return
      end select
      ...
      call mach_cam_aerocld(throw, xrow, rhsize, aeronum, thlev, roarow, &
                            ...
                            wetflx, fctr, adj_ctr, frevp, rhrow, ccn, kount, pni, pnk)
...
!  section 8: diagnostic outputs
   ...
   call msg_toall(chm_msg_debug, 'mach_cam_main [END]')
