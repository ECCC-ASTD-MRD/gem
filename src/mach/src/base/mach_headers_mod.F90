...
subroutine mach_biog_main(pvars, metvar2d, metvar3d, iseasons,  metvar3dcan, &
...
subroutine mach_input_check(pvars, metvar2d, metvar3d, landuse)
   use phymem,               only: phyvar
   use chm_ptopo_grid_mod,   only: chm_ni, chm_nk
   use chm_metvar_mod,       only: SIZE_MV2D, SIZE_MV3D
...
subroutine mach_plumerise(pvars, chem_tr, metvar2d, metvar3d, landuse, &
                          slab_index)
   use phymem,                  only: phyvar
   use chm_metvar_mod,          only: SIZE_MV2D, SIZE_MV3D
   use chm_ptopo_grid_mod,      only: chm_ni, chm_nk
...
subroutine mach_vit_diffusivity(pvars, dxdy, zmom, kvkt, nmod, ni)
   use phymem,               only: phyvar
   use chm_ptopo_grid_mod,   only: chm_nk
...
subroutine mach_stepinit(pvars, step, trnch, ni_can)
   use phymem,               only: phyvar
 ...
 subroutine mach_lai_adjust(pvars, landuse, trnch)
   use phymem,               only: phyvar
   use chm_ptopo_grid_mod,   only: chm_ni
   use mach_drydep_mod,      only: lucprm
   integer(kind=4), intent   (in) :: trnch
