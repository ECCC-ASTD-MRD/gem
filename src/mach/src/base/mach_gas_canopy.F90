...
!
!!if_on
subroutine mach_gas_canopy(pvars, chem_tr, tracers_can, metvar2d, &
...
!
   !-----------------------------------------------------------------
   call msg_toall(chm_msg_debug, 'mach_gas_canopy [BEGIN]')
...
! Fraction of cumulative LAI above each canopy layer interface, counting down from top:
      laicumfrac(1) = pvars(sm(sp_CRL)%per_pvarid)%data(ii)
      laicumfrac(2) = pvars(sm(sp_CRL + 1)%per_pvarid)%data(ii)
      laicumfrac(3) = pvars(sm(sp_CRL + 2)%per_pvarid)%data(ii)
      laicumfrac(4) = pvars(sm(sp_CRL + 3)%per_pvarid)%data(ii)
!
!  Use BELD3 data for clumping index instead of the 15 land use categories:
      klight = 0.5 * pvars(sm(sp_CLU)%per_pvarid)%data(ii)
!
...
!
      coeff = klight * pvars(sm(sp_LAI)%per_pvarid)%data(ii) / &
              max(0.05, csza(ic))
      ...
      zbio (1) = pvars(sm(sp_HC)%per_pvarid)%data(ii)
      zbio (2) = zbio(1) * 0.75
      zbio (3) = zbio(1) * 0.50
      zbio (4) = zbio(1) * 0.35
      zbio (5) = zbio(1) * 0.20
...
! (i): Model resolved layers:
         do kk = 1, chm_nk
            k = kmod(ic, kk)
            this_ik = ik(ii, kk, chm_ni) + 1
            hstart(ic, k) = dble(pvars(sm(var_step)%per_pvarid)%data(this_ik))
         end do
! (ii): Canopy shaded layers:
         do kc = 1, nkc
            k = kcan(ic, kc)
            this_ik = ik(ii, kc, chm_ni) + 1
            hstart(ic, k) = dble(pvars(sm(var_step)%dd_pvarid)%data(this_ik))
         end do
      end do
   end if
!
...
! (i): Model resolved layers:
         do kk = 1, chm_nk
            k = kmod(ic, kk)
            this_ik = ik(ii, kk, chm_ni) + 1
            pvars(sm(var_step)%per_pvarid)%data(this_ik) = real(hstart(ic, k))
         end do
! (ii): Canopy shaded layers:
         do kc = 1, nkc
            k = kcan(ic, kc)
            this_ik = ik(ii, kc, chm_ni) + 1
            pvars(sm(var_step)%dd_pvarid)%data(this_ik) = real(hstart(ic, kc))
         end do
      end do
   end if
!
   ...
   if (sp_JNO2 > 0) then
      ...
      do ic = 1, ni_can
         ...
         do kk = 1, chm_nk
            k = kmod(ic, kk)
            ik0 = ik(ic+1, k, ni_can)
            pvars(sm(sp_JNO2)%out_pvarid)%data(imod(ic)) = real(rjval(ik0, 1))
         end do
      end do
   end if
!
...
!            this_ik = ik(ii, kk, chm_ni) + 1
!            ik0 = ik(ic+1, k, ni_can)
!            pvars(sm(dbg_3d(1))%out_pvarid)%data(this_ik) = real(rjval(ik0, 1))  !jcor JNO2
!            pvars(sm(dbg_3d(2))%out_pvarid)%data(this_ik) = real(rjval(ik0, 5))  !jcor JO3=JO1d
!            pvars(sm(dbg_3d(3))%out_pvarid)%data(this_ik) = real(rjval(ik0, 2))  !jcor JNO3
!            pvars(sm(dbg_3d(4))%out_pvarid)%data(this_ik) = real(rjval(ik0, 7))  !jcor JHNO3
!            pvars(sm(dbg_3d(5))%out_pvarid)%data(this_ik) = real(rjval(ik0, 9))  !jcor JH2O2
!            pvars(sm(dbg_3d(6))%out_pvarid)%data(this_ik) = real(rjval(ik0, 4))  !jcor JO3p
!         end do
!      end do
!   end if
!
...
! (i): Model resolved layers:
        ...
        ! convert dsoa from ug/m3 to ug/kg and output to vol bus
            pvars(sm(sp_NWOC)%out_pvarid)%data(this_ik) = dsoa(ic, k) / rho(ic, k)
         end do
! (ii): Canopy shaded layers:
! Note the use of sm(sp_NWOC)%vd_pvarid) here, to hold the SOA yields within canopy
         do kc = 1, nkc
            k = kcan(ic, kc)
            this_ik = ik(ii, kc, chm_ni) + 1
            pvars(sm(sp_NWOC)%vd_pvarid)%data(this_ik) = dsoa(ic, k) / rho(ic, k)
         end do
!
      end do
!
...
!      Evaluate LINOZ O3 tendency locally
         call mach_gas_strato(pvars, hu_ppm, tplus, sigt, o3_old, psurf, &
...
!
      IF_DIAG: if (chm_diag_colum_L) then
         ...
         ! Ozone tendency
               ...
               pvars(sm(sp_tend_o3)%out_pvarid)%data(this_ik) = o3_tend
               pvars(sm(sp_diff_o3)%out_pvarid)%data(this_ik) = o3_tend
            end do
         end do
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_gas_canopy [END]')
