...
subroutine mach_incld_main(gaz_conc, aerocon, q_bin, tempk, psacw, rad1, rcrit,&
                           roarow, ibulk, flux, fctr, adj_ctr, aeronum, pni, pnk)
...
#include <rmn/msg.h>
   !-----------------------------------------------------------------
   call msg_toall(chm_msg_debug, 'mach_incld_main [BEGIN]')
   if (chm_timings_L) call timing_start_omp(350, 'mach_incld_main', 340)
...
!  The original gaseous species concentrations were stored in gtmp.
   ...
   call msg_toall(chm_msg_debug, 'mach_incld_main [END]')
