...
!================================================================================================
!!if_on
subroutine mach_gas_strato(pvars, hu_ppm, ztplus, sigt, o3, zpplus, lino3_new, &
                           gni, gnk, nmod)
...
!!if_off
   ...
   use chm_species_idx_mod , only: sp_O3,  lin_c2, lin_c4, lin_c5, lin_c6, &
                                   lin_c7
   use linoz_param,          only: p_linoz_meso, p_linoz_c4, QepsO3
   use linoz_tend_mod,       only: linoz_tend
   use linoz_xcol,           only: linoz_xcol1
...
!
   ...
   do k = 1, gnk
      do i = 1, gni
         this_ik = ik(nmod(i), k, chm_ni) + 1
         zttce(i, k) = pvars(sm(lin_c2)%per_pvarid)%data(this_ik)
         zlin4(i, k) = pvars(sm(lin_c4)%per_pvarid)%data(this_ik)
         zlin5(i, k) = pvars(sm(lin_c5)%per_pvarid)%data(this_ik)
         zlin6(i, k) = pvars(sm(lin_c6)%per_pvarid)%data(this_ik)
         zlin7(i, k) = pvars(sm(lin_c7)%per_pvarid)%data(this_ik)
         ...
         zo3s  = pvars(o3s)%data(this_ik)
         o3c_vmr(i, k) = zo3s * ugkg2vmr                  !mole /mole vmr  <-- kg /kg

         zo3ce = pvars(o3ce)%data(this_ik)
   ...
   call linoz_xcol1(o3_vmr, pbot, ptop, zo3col, gni, gnk)

      ! Total column ERA5-FK-HALOE ozone climatology (D.U.)
   call linoz_xcol1(o3c_vmr, pbot, ptop, zo3ccol, gni, gnk)
!
      !    Linoz tendencies
   cnull = 0.0
   nullify(cnullptr)
   ...
   call linoz_tend(                                         &
           ...
           cnull                               , & ! o8_tend input mole/mole sec-1
           lino3_new                                      , & !output, mole /mole
           cnull ,cnull, cnull ,cnull                     , & !output, mole /mole /sec
           cnullptr                            , & ! zo8chmtd output, mole /mole /sec
