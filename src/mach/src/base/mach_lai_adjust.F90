...
!!if_off
   ...
   use chm_species_idx_mod,  only: sp_LAI, sp_STSE, ce_LAI
...
!  External subroutines:
   ...
   istat = phymem_find(idxv1, chem_ent_vars(ce_LAI)%ent_name, &
           F_npath='V', F_bpath='E', F_quiet=.false., F_shortmatch=.false.)
   if (istat < 0) then
...
!  Determine current LAI relative to grid-cell maximum and minimum
      ...
      if (laimax > 0.0 .and. (laimax - laimin) > 0.0001) then
         pvars(sm(sp_STSE)%per_pvarid)%data(ii) = &
                             (lai_mnt(imonth, ii) - laimin) / (laimax - laimin)
      else
         pvars(sm(sp_STSE)%per_pvarid)%data(ii) = 1.0
      end if
!
!  save the current month's LAI field in the permanent bus for subsequent calculations
      pvars(sm(sp_LAI)%per_pvarid)%data(ii) = lai_mnt(imonth, ii)
!
   end do
