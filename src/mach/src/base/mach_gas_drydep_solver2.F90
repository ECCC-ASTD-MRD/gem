...
! Fichier / File   : mach_gas_drydep_solver2.ftn90
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_gas_drydep_solver2 [BEGIN]')
...
!&&&&&&&&&&&&&&&&&&&   end of ocean block &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
            ...
            if (vdf < 0.0) then
               if(local_dbg) &
                   write(chm_lun_out, *) '>Warning Dry deposition speed negative reset to zero'
               vdf = 0.0
            end if
...
!           if ((ylat .le. 40.0) .and. (srad < 75.0) .and. (ylon.lt.160.0) .and. (nlus .le.12)) vdf=1.0E-5
   ...
   call msg_toall(chm_msg_debug, 'mach_gas_drydep_solver2 [END]')
