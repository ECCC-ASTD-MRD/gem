...
!
!!if_on
subroutine mach_gas_bidi(pvars, chem_tr, metvar2d, metvar3d, vdg)
...
!!if_off
   ...
   use chm_utils_mod,        only: chm_timestep, ik, chm_msg_debug
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_gas_bidi [BEGIN]')
   ...
   do i = 1, chm_ni
      ...
      if (gepdyn) then
         ...
         pvars(sm(sp_NH3)%epa_pvarid)%data(i) = xa / coeff  ! diagnostic atmospheric deposition potential
      ...
      do nlus = 1, lucprm
         ...
         this_ik = ik(i, nlus, chm_ni) + 1
         if (chm_nh3_gep2d_l) then
            gamma_s = pvars(sm(sp_NH3)%gep_pvarid)%data(this_ik)
         else
            gamma_s = chm_nh3_gep(nlus)
         end if

         if (gepdyn) then
            gamma = pvars(sm(sp_NH3)%epd_pvarid)%data(this_ik)
         else
            gamma = gamma_s
         end if
         ...
         if (gepdyn) then
            ...
            if (chm_soil_ph2d_l) then
               pH = pvars(sm(sp_NH3)%sph_pvarid)%data(this_ik)
            else
               pH = chm_soil_ph(nlus)
            end if
            ...
            pvars(sm(sp_NH3)%epd_pvarid)%data(this_ik) = gamma + chm_timestep * &
                         ( lambda_a * (xa - xg) + (gamma_s - gamma) / tau_soil )
            ...
            pvars(sm(sp_NH3)%bdt_pvarid)%data(this_ik) = tau_a

         end if

      end do

      pvars(sm(sp_NH3)%bd_pvarid)%data(i) = emis

   end do

   call msg_toall(chm_msg_debug, 'mach_gas_bidi [END]')
