...
!
      ...
      real(kind=4)  :: Le, T0, Tm, Tt, d, theta1, theta2,     &
                       Mc, T1, T2, T3, T4, Td, fQ, pQatm, &
...
!/*        Newer approach (fix to cover inversions)     2024-02-09    */
         qb = -cpd * log((theta1)/(theta2)) * ((T1+T2-T3-T4)/2.)
...
!        by 1 metre until Qatm < Qt again, in which case the soluiton is reached. */
         if (iConverge == 0 .and. Qatm < (Qt + Qw) .and. i + 1 < MET_LEVELS) then
            i = i + 1
            pQatm = Qatm
            if (trim(cffeps_fire_type) == "wet") then
         ...
         else if (i + 1 >= MET_LEVELS) then  ! maxed out the levels
            iConverge = -1
