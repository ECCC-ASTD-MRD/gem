...
!
!!if_on
subroutine mach_diff_prep(pvars, chem_tr, metvar2d, metvar3d, &
...
!
      ...
      do k = 1, chm_nk
         do i = 1, ni_nocan
            ii = imod2(i)
            this_ik = ik(ii, k, chm_ni) + 1
            kt(i, k) = pvars(sm(sp_KTN)%out_pvarid)%data(this_ik)
!
...
!  velocities for PMs are handled by the CAM package.
      vd = 0.0
      do sp = 1, nb_dyn_tracers
         if (sm(sp)%vd_pvarid > 0) then
            do i = 1, ni_nocan
               vd(i, sp) = pvars(sm(sp)%vd_pvarid)%data(imod2(i))
            end do
!
!  Depostion flux diagnostic for gas -- HARDCODE negative
            if (sm(sp)%dd_pvarid > 0) then
               do i = 1, ni_nocan
                  ii = imod2(i)
                  flux = chm_timestep * rho(i, chm_nk) * conc(i, chm_nk, sp) * &
                         vd(i, sp) * 1.e-6 / sm(sp)%mol_wt
                  pvars(sm(sp)%dd_pvarid)%data(ii) = - flux + &
                                         pvars(sm(sp)%dd_pvarid)%data(ii)
               end do
            end if
         end if
!
...
!  Non on-road mobile anthropogenic/biogenic area sources.
      ...
      call mach_diffusion(pvars, conc, vd, psurf, dxdy, rho, kt, vt, &
...
!
         ...
         ! original diffusivities and species concentration on canopy levels
            this_ik = ik(ii, k, chm_ni) + 1
            kt_can(ic, kk) = pvars(sm(sp_KTN)%out_pvarid)%data(this_ik)
            do sp = 1, nb_dyn_tracers
...
!  Evaluate the vehicle induced turbulence diffusivities (kvkt)
      ...
      kvkt_can  = 0.0
      if (chm_vit_l) then
         call mach_vit_diffusivity(pvars, dxdy_can, zmomcan, kvkt_can0, imod, &
                                   ni_can)
      end if
!
      do ic = 1, ni_can
...
!  Deposition velocities placed in deposition velocity array.
      ...
      do sp = 1, nb_dyn_tracers
         if (sm(sp)%vd_pvarid > 0) then
            do ic = 1, ni_can
               vd_can(ic, sp) = pvars(sm(sp)%vd_pvarid)%data(imod(ic))
            end do
!
!  Depostion flux diagnostic for gas - HARDCODE negative
            if (sm(sp)%dd_pvarid > 0) then
...
!  (1) Zero emissions solution with original diffusivities.
      echoice = 0
      call mach_diffusion(pvars, conc0_can, vd_can, psurf_can, &
...
!
      echoice = 3
      if (chm_vit_l) echoice = 1
      call mach_diffusion(pvars, conc_can, vd_can, psurf_can, dxdy_can,&
...
!  increment in concentration due to ae_pvarid, fae_pvarid, and be_pvarid emissions if VIT
!  is "on", and ae_pvarid, fae_pvarid, be_pvarid, and mae_pvarid if VIT is "off".
!
      calculate = .false.
      do sp = 1, nb_dyn_tracers
!  Check to see if ae_pvarid, fae_pvarid or be_pvarid species are present and have
!  non-zero emissions.  These are needed for both vit and non-vit simulations.
        if (sm(sp)%ae_pvarid > 0) then
          do ic = 1, ni_can
            if(pvars(sm(sp)%ae_pvarid)%data(imod(ic)) > 0.0) calculate(ic,sp) = .true.
          end do
        end if
        if(sm(sp)%fae_pvarid > 0) then
          do ic = 1, ni_can
            if(pvars(sm(sp)%fae_pvarid)%data(imod(ic)) > 0.0) calculate(ic,sp) = .true.
          end do
        end if
        if(sm(sp)%be_pvarid > 0) then
          do ic = 1, ni_can
            if(pvars(sm(sp)%be_pvarid)%data(imod(ic)) > 0.0) calculate(ic,sp) = .true.
          end do
        end if
!  Check for mae_pvarid emissions being present ? only needed if VIT option is not used,
!  and mobile area source is being added as normal surface flux with no KT changes:
        if((.not. chm_vit_l) .and. (sm(sp)%mae_pvarid > 0)) then
          do ic = 1, ni_can
            if(pvars(sm(sp)%mae_pvarid)%data(imod(ic)) > 0.0) calculate(ic,sp) = .true.
          end do
         end if
      end do
...
!  Add up the VIT and meteorological induced KT
         ...
         echoice = 2
         call mach_diffusion(pvars, conc_mob_can, vd_can, psurf_can, &
...
!  Calculate contribution due to on-road mobile anth area sources:
         do sp = 1, nb_dyn_tracers
            if (sm(sp)%mae_pvarid > 0) then
               do ic = 1, ni_can
!  Calculate increment only if species had non-zero emissions.
                  if (pvars(sm(sp)%mae_pvarid)%data(imod(ic)) > 0.0) then
                     do kk = 1, nkt
...
!  individual model layers:
      echoice = -1
      call mach_diffusion(pvars, conc_bio_can, vd_can, psurf_can, &
           dxdy_can, rho, kt_can, vt_can, sigm, sigt, zplus, zmom, echoice,&
