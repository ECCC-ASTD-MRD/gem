...
!                  called once via rpnphy api/phy_nml
! Extra info     :
!
! Arguments:
...
!!if_off
   use chm_utils_mod,      only: chm_lun_out, undefined, chm_stop
   use chm_nml_mod
...
!
   integer(kind=4), external :: fnom, fclos
!
   chm_lun_out = lun_out
!
   if (chm_lun_out > 0) then
      write(chm_lun_out, *) ''
      write(chm_lun_out, *) 'Begin reading namelist chemistry_cfgs (s/r chm_nml)'
   end if
!
...
!
   ...
   if (read_status > 0) then
      if (chm_lun_out > 0) then
         write(chm_lun_out, *) ' Error in reading namelist chemistry_cfgs (s/r chm_nml)'
...
!     =========
      if (chm_cffeps_online_l) then
         istatus = min(mach_cffeps_nml(unf), 1, istatus)
      end if
