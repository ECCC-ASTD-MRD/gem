...
!
   do i = 1, chm_ni
      nn = sp_be_std
      do nsp = 1, num_be_std
         summer_std_p(nsp, i) = pvars(sm(nn)    %per_pvarid)%data(i)
         winter_std_p(nsp, i) = pvars(sm(nn + 1)%per_pvarid)%data(i)
         nn = nn + 2
      end do
...
!  sesn_coef based on proximity to maximum monthly LAI:
      do i = 1, chm_ni
         sesn_coef(i) = pvars(sm(sp_STSE)%per_pvarid)%data(i)
      end do
   else
!  sesn_coef based on spatial mask inherited from ADOM model (?)
   ...
   if (chm_canopy_shading_l) then
      do i = 1, chm_ni
         canopy_column(i) = pvars(sm(sp_FRT)%per_pvarid)%data(i) > 0.0
      end do
   else
      canopy_column = .false.
      canopy_hgt    = 0.0
...
!
   ...
   ic = 0
   do i = 1, chm_ni
      lai_p = pvars(sm(sp_LAI)%per_pvarid)%data(i)
!
      if (canopy_column(i)) then
! Fraction of LAI at heights of 1.0, 0.5 and 0.2 canopy height ,
! counting down from top:
         laifrac(1, i) = pvars(sm(sp_FRL)%per_pvarid)%data(i)
         laifrac(2, i) = pvars(sm(sp_FRL + 1)%per_pvarid)%data(i)
         laifrac(3, i) = pvars(sm(sp_FRL + 2)%per_pvarid)%data(i)
         laifrac(4, i) = 1.0
!
...
! Return the biogenic emissions to the bus
   do nsp = 1, num_be_sp
      nn = be_species(nsp)
      do i = 1, chm_ni
         pvars(sm(nn)%be_pvarid)%data(i) = emisbio(nsp, i)
      end do
   end do
