...
!
!!if_on
subroutine mach_output(pvars, chem_tr, metvar2d, metvar3d, landuse, trnch)
...
!!if_off
   ...
   use chm_utils_mod,        only: ik, chm_msg_debug, chm_stop
   ...
   use phymem,               only: phymem_getdata
   implicit none
!!if_on
...
!
   ...
   integer(kind=4)            :: busid, busid2,istat
   real(kind=4), pointer, contiguous :: bptr(:), bptr2d(:,:), bptr2d2(:,:)
...
! Evaluate air density at the model surface (diagnostic level)
   rhod(:) = metvar2d(:, MV2D_PPLUS) / (rgasd * metvar2d(:, MV2D_TDIAG) * &
                                   (1.0 + delta * metvar2d(:, MV2D_QDIAG)))

! 1) Units conversion for gases     (ug/kg) --> (ppb)
! =======================================================================================================
! NOy:
   if (sp_NOy > 0) then
      busid = sm(sp_NOy)%out_pvarid
      if (busid > 0) then
         nullify(bptr2d)
         istat = phymem_getdata(bptr2d, busid, trnch)
         if (istat < 0) call chm_stop('error loading bptr2d', istat)
         bptr2d(:, :) = 0.0
         do nsp = 1, nb_NOy_species
            isp = NOy_species_list(nsp)
            factor = mwt_air / sm(isp)%mol_wt
            bptr2d(:, :) = bptr2d(:, :) + chem_tr(:, :, isp) * factor
         end do
      end if
   end if
!
...
! Default: O3, NO, NO2, SO2:
   do nsp = 1, nsp_gas_ppb_out
      ...
      busid = sm(isp)%out_pvarid
      nullify(bptr2d)
      istat = phymem_getdata(bptr2d, busid, trnch)
      if (istat < 0) call chm_stop('error loading bptr2d', istat)
      factor = mwt_air / sm(isp)%mol_wt
      bptr2d(: ,:) = chem_tr(:,:,isp) * factor
   end do
   ...
   if (chm_pkg_pm_s(1:3) == 'CAM') then
      do ibin = 1, isize
         do iae = 1, icom
            ...
            do i = 1, chm_ni ! copy surface to diagnostic layer
...
! Initializations;
      ...
      spm10 = 0.0
      spmt  = 0.0
      pvars(sm(sp_AF)%out_pvarid)%data = 0.0
      pvars(sm(sp_AC)%out_pvarid)%data = 0.0
!
...
! 3b) Evaluate PM2.5(AF) and PM10(AC) (by aggregating spm25 and spm10 across species)
      do iae = 1, icom
         ...
         nullify(bptr2d)
         istat = phymem_getdata(bptr2d, sm(sp_AF)%out_pvarid, trnch)
         if (istat < 0) call chm_stop('error loading bptr2d', istat)
         bptr2d(:, :) = bptr2d(:, :) + spm25(:, :, iae)

         nullify(bptr2d)
         istat = phymem_getdata(bptr2d, sm(sp_AC)%out_pvarid, trnch)
         if (istat < 0) call chm_stop('error loading bptr2d', istat)
         bptr2d(:, :) = bptr2d(:, :) + spm10(:, :, iae)
      end do
!
...
! Total aerosol number concentration per bin
         do ibin = 1, isize
            busid = sm(sp_NUMC + ibin - 1)%out_pvarid
            nullify(bptr2d)
            if (busid > 0) istat = phymem_getdata(bptr2d, busid, trnch)
            if (istat < 0) call chm_stop('error loading bptr2d', istat)
            do k = 1, chm_nk + 1
               do i = 1, chm_ni
                  aerovol = 0.0
...
! PM2.5 accumulators
         pvars(sm(sp_NAF)%out_pvarid)%data = 0.0
         pvars(sm(sp_NAC)%out_pvarid)%data = 0.0
         pvars(sm(sp_NAT)%out_pvarid)%data = 0.0
!
         do ibin = 1, isize
            if (aerosize(ibin + 1) < maxradius_pm25) then
               busid = sm(sp_NAF)%out_pvarid
               nullify(bptr2d)
               istat = phymem_getdata(bptr2d, busid, trnch)
               if (istat < 0) call chm_stop('error loading bptr2d', istat)
               bptr2d(:, :) = bptr2d(:, :) + aeronum(:, :, ibin)
! Partial PM2.5 and PM10 accumulators
            else if (aerosize(ibin + 1) == maxradius_pm25) then
               volratpm2p5 = (1.25**3 - aerosize(ibin)**3) / &
                             (aerosize(ibin+1)**3 - aerosize(ibin)**3)
               busid = sm(sp_NAF)%out_pvarid
               busid2 = sm(sp_NAC)%out_pvarid
               nullify(bptr2d, bptr2d2)
               istat = phymem_getdata(bptr2d, busid, trnch)
               if (istat < 0) call chm_stop('error loading bptr2d', istat)
               bptr2d(:, :) = bptr2d(:, :) + aeronum(:, :, ibin) * volratpm2p5

               istat = phymem_getdata(bptr2d2, busid2, trnch)
               if (istat < 0) call chm_stop('error loading bptr2d2', istat)
               bptr2d2(:, :) = bptr2d2(:, :) + bptr2d(:, :) + &
                               aeronum(:, :, ibin) * (1.0 - volratpm2p5)
! PM10 accumulators
            else if ((aerosize(ibin + 1) > maxradius_pm25) .and. &
                     (aerosize(ibin + 1) < maxradius_pm10)) then
               busid = sm(sp_NAC)%out_pvarid
               nullify(bptr2d)
               istat = phymem_getdata(bptr2d, busid, trnch)
               if (istat < 0) call chm_stop('error loading bptr2d', istat)
               bptr2d(:, :) = bptr2d(:, :) + aeronum(:, :, ibin)
! Partial PM10 and PMtot accumulators
               ...
               volratpm10 = (5.0**3 - aerosize(ibin)**3) / &
                            (aerosize(ibin+1)**3 - aerosize(ibin)**3)
               busid = sm(sp_NAC)%out_pvarid
               busid2 = sm(sp_NAT)%out_pvarid
               nullify(bptr2d, bptr2d2)
               istat = phymem_getdata(bptr2d, busid, trnch)
               if (istat < 0) call chm_stop('error loading bptr2d', istat)
               bptr2d(:, :) = bptr2d(:, :) + aeronum(i, k, ibin) * volratpm10
...
! PMtot accumulators:
               ...
               busid = sm(sp_NAT)%out_pvarid
               nullify(bptr2d)
               istat = phymem_getdata(bptr2d,sm(sp_NAT)%out_pvarid, trnch)
               if (istat < 0) call chm_stop('error loading bptr2d', istat)
               bptr2d(:, :) = bptr2d(:, :) + aeronum(i, k, ibin)
            end if
         end do ! End ibin loop
...
! ===============================================================================
      if (chm_aqhi_l) then
         do k = 1, chm_nk + 1
            do i = 1, chm_ni
               this_ik = ik(i, k, chm_ni) + 1
               no2_part = exp(0.000871 * chem_tr(i, k, sp_NO2) * mwt_air / &
                                        sm(sp_NO2)%mol_wt) - 1.0
               o3_part  = exp(0.000537 * chem_tr(i, k, sp_O3) * mwt_air / &
                                        sm(sp_O3)%mol_wt) - 1.0
               pm25_part = exp(0.000487 * pvars(sm(sp_AF)%out_pvarid)%data(this_ik)) - 1.0
               pm10_part = exp(0.000297 * pvars(sm(sp_AC)%out_pvarid)%data(this_ik)) - 1.0

               pvars(sm(sp_AQ25)%out_pvarid)%data(this_ik) = (10. / 10.4) * &
                                      100. * (no2_part + o3_part + pm25_part)
               pvars(sm(sp_AQ10)%out_pvarid)%data(this_ik) = (10. / 11.7) * &
...
! =============================================================================
   if (sp_NOy > 0) then
      if (sm(sp_NOy)%dd_pvarid > 0) then
         dryflx(:) = 0.0
         do nsp = 1, nb_NOy_species
            isp = NOy_species_list(nsp)
      !Only include NOy species with defined deposition field
            if (sm(isp)%dd_pvarid <= 0) cycle
            dryflx(:) = dryflx(:) + pvars(sm(isp)%dd_pvarid)%data
         end do
         pvars(sm(sp_NOy)%dd_pvarid)%data(:) = dryflx(:)
      end if
   end if
...
! ==============================================================================
   if (sp_density > 0) then
      nullify(bptr2d)
      istat = phymem_getdata(bptr2d, sm(sp_density)%out_pvarid, trnch)
      if (istat < 0) call chm_stop('error loading bptr2d', istat)
      bptr2d(:, 1:chm_nk) = metvar3d(:, 1:chm_nk, MV3D_RHO)
      bptr2d(:, chm_nk+1) = rhod(:) ! diagnostic level
   end if
...
! ==============================================================================
   if (chm_lu15_out_l) then
      nullify(bptr2d)
      istat = phymem_getdata(bptr2d, sm(sp_LU15)%per_pvarid, trnch)
      if (istat < 0) call chm_stop('error loading bptr2d', istat)
      do k = 1, lucprm
         bptr2d(:, k) = landuse(:, k)
      end do
