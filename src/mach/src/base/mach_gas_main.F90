...
!
!!if_on
subroutine mach_gas_main(pvars, chem_tr, metvar2d, metvar3dnocan, &
...
!
   !-----------------------------------------------------------------
   call msg_toall(chm_msg_debug, 'mach_gas_main [BEGIN]')
   ...
   if (var_step > 0) then
      do k = 1, chm_nk
         do i = 1, ni_nocan
            ...
            hstart(i, k) = dble(pvars(sm(var_step)%per_pvarid)%data(this_ik))
         end do
      end do
   end if
!
   ...
   if (var_step > 0) then
      do k = 1, chm_nk
         do i = 1, ni_nocan
            ii = imod2(i)
            this_ik = ik(ii, k, chm_ni) + 1
            pvars(sm(var_step)%per_pvarid)%data(this_ik) = real(hstart(i, k))
         end do
      end do
   end if
!
   ...
   if (sp_JNO2 > 0) then
      ...
      do i = 1, ni_nocan
         ...
         do k = 1, chm_nk
            ...
            pvars(sm(sp_JNO2)%out_pvarid)%data(imod2(i)) = real(rjval(ik0, 1))
         end do
      end do
   end if
!
...
!
      do k = nk_start, chm_nk
        ...
        ! convert dsoa from ug/m3 to ug/kg and output to vol bus
            this_ik = ik(imod2(i), k, chm_ni) + 1
            pvars(sm(sp_NWOC)%out_pvarid)%data(this_ik) = &
                                                    dsoa(i, k) / rho(i, k)
         end do
      end do
!
...
!
   ...
   call msg_toall(chm_msg_debug, 'mach_gas_main [END]')
