...
!!if_off
   ...
   use chm_metvar_mod,      only: MV2D_CANG, MV3D_TPLUS, MV2D_PPLUS, MV3D_HUPLUS, &
                                  MV3D_LWCRAD, MV2D_MG, MV3D_CLDRAD, MV2D_AL5,    &
   ...
   use suncos,              only: suncos1  ! RPNPHY
...
!! cloud liquid path follow GEM "prep_cw_rad.F90",  kg/m2->g/m2
         liqwcin = max(gmetvar3d(i, k, MV3D_LWCRAD), 0.0) / &
...
! recalculate cos(sza) since value from GEM is limited to 0-90deg.
   ...
   call suncos1(csza, gni, dlat, dlon, hrnow, njul_day)
...
!
         ...
         do k = 1, gnk
            do i = 1, gni
               ...
               rj(ik0, 22) = jval( jval_NO2     ,i,k) * 2.35E-4 / 0.723 !RNO3 + hv = 0.344HO2+0.554RO2R+NO2+0.102RO2N+0.167R2O2+0.135HCHO+0.444CCHO+0.137RCHO+0.528PROD2+0.786XC -- by Craig
