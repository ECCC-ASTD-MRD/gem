...
subroutine svs2(BUS, BUSSIZ, PTSURF, PTSURFSIZ, DT, KOUNT, N, M, NK)
...
#include <rmnlib_basics.hf>
   ...
   !@Author S. Belair (January 1997), M. Abrahamowicz, S.Z. Husain (2012)
   !@Revisions
   ! 001      Rewrite ISBA into SVS (multi-budget, multi-layer isba)
   !              Add sea ice surface
   !@Object Multitasking of the surface scheme SVS
   !@Arguments
   !               - Input/Output -
...
!   In this case, min_wind_speed is really numeric = VAMIN
      ...
      call snow_veg1(bus(x(TSNOWVEG  ,1,1)), bus(x(TSNOWVEG,1,2)),  &
                     bus(x(SNVRO     ,1,1)),   &
      ...
      call ebudget_svs1(bus(x(TSA ,1,1)),  &
