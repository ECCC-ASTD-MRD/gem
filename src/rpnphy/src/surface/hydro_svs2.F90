...
!!!#include <arch_specific.hf>
  ...
  real, dimension(n,nl_svs) :: bcoef, fbcof, ksat
  real, dimension(n,nl_svs) :: psisat, wfcint, wsat, etr_grid
  real, dimension(n)        :: grkef, eg_grid
  ! prognostic vars (I/0)
  real, dimension(n,nl_svs) :: wd , wdt, wf, wft
  ! output
  real, dimension(n,nl_svs) :: grksat, ksatc, ksatc_rk2, ksatc_rk3
  ...
  !Revisions
  !
  ! 001      Bug fix for RK4 (N.Leroux, Jan 2026)
  !Object
  ...
  !
  ! ETR_GRID Evapotranspiration from each layer (grid box average) [m/s]
  ! EG_GRID  evaporation rate over bare ground and bare ground below high veg (grid box average) [kg/m2/s]
...
! EG_code related to ponding of water
  ...
  !        3.      CALCULATE BASEFLOW AT THE BOTTOM OF THE TOTAL SOIL PROFILE
  !              -----------------------------------------
  ...
  !        4.      CALCULATE NEW SOIL WATER CONTENT FOR EACH LAYER
  !              -----------------------------------------
  !
  !
  ...
  DO I=1,N
     F(I,1) = (PG(I) - EG_GRID(I)) * DT / 1000. ! to m
  END DO
  ...
  !        6.     TIME-EVOLUTION OF THE FROZEN SOIL WATER
  !               ---------------------------------------
  !
