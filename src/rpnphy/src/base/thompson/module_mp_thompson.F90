...
!#TODO: get Open units from fnom to avoid collisions
...
!.. specify a name of external file(s) including version number for pre-computed
      ...
      character(len=512), save, public :: modelinput = "/fs/ssm/eccc/mrd/rpn/MIG/GEM/d/gem-data/gem-data_4.2.0/gem-data_4.2.0_all/share/data/dfiles/datafiles/modeldata/"
      
      character(len=*), parameter :: basepath = 'thompson/v_1.0.0/'
      ! character(len=*), parameter :: thomp_table_file = 'thompson_tables_precomp_v2.sl'  !#TODO never used?
      ! character(len=*), parameter :: qr_acr_qg_file    = '/home/apm000/ords/datafiles/constants/Thompson/v_1.0.0/qr_acr_qgV2.dat'
      ! character(len=*), parameter :: qr_acr_qs_file    = '/home/apm000/ords/datafiles/constants/Thompson/v_1.0.0/qr_acr_qsV2.dat'
      ! character(len=*), parameter :: freeze_h2o_file   = '/home/apm000/ords/datafiles/constants/Thompson/v_1.0.0/freezeH2O.dat'
      ! character(len=*), parameter :: ccn_activate_file = '/home/apm000/ords/cmdn/Thompson_MP/test/test_output/CCN_ACTIVATE.BIN'
      character(len=*), parameter :: qr_acr_qg_file    = basepath//'qr_acr_qgV2.dat'
      character(len=*), parameter :: qr_acr_qs_file    = basepath//'qr_acr_qsV2.dat'
      character(len=*), parameter :: freeze_h2o_file   = basepath//'freezeH2O.dat'
      character(len=*), parameter :: ccn_activate_file = basepath//'CCN_ACTIVATE.BIN'

      INTEGER, PARAMETER, PRIVATE:: R8SIZE = REAL64
      INTEGER, PARAMETER, PRIVATE:: R4SIZE = REAL32
...
!!$            end if
!!$         endif
         if (.not.is_aerosol_aware .and. .not.merra2_aerosol_aware .and. &
...
!+---+-----------------------------------------------------------------+
      do k = kts, kte
         orho = 1./rho(k)
         IF1: if ( (ssatw(k).gt. eps) .or. (ssatw(k).lt. -eps .and. &
                   L_qc(k)) ) then
          ...
          xrc = rc(k) + clap*rho(k)
          xnc = 0.
          IF2: if (xrc.gt. R1) then
           prw_vcd(k) = clap*odt
!+---+-----------------------------------------------------------------+ !  DROPLET NUCLEATION
           IF3: if (clap .gt. eps) then
!!$            if (is_aerosol_aware .or. merra2_aerosol_aware) then
...
!+---+
      ...
      good = 0
        INQUIRE(FILE=trim(modelinput)//qr_acr_qg_file, EXIST=lexist)
!!$#ifdef MPI
!!$        call MPI_BARRIER(mpi_communicator,ierr)
!!$#endif
        IF ( lexist ) THEN
          call msg(MSG_INFO,'(Thompson) reading: '//trim(modelinput)//qr_acr_qg_file)
          OPEN(63,file=trim(modelinput)//qr_acr_qg_file,form="unformatted",err=1234)
!sms$serial begin
...
!.. fortran indices.  J. Michalakes, 2009Oct30.
 ...
 97            continue
        ...
        IF ( write_thompson_tables ) THEN
          write(0,*) "Writing "//trim(modelinput)//qr_acr_qg_file//" in Thompson MP init"
          OPEN(63,file=trim(modelinput)//qr_acr_qg_file,form="unformatted",err=9234)
...
!+---+
      ...
      good = 0
        INQUIRE(FILE=trim(modelinput)//qr_acr_qs_file, EXIST=lexist)
!!$#ifdef MPI
!!$        call MPI_BARRIER(mpi_communicator,ierr)
!!$#endif
        IF ( lexist ) THEN
          !write(0,*) "ThompMP: read "//qr_acr_qs_file//" instead of computing"
          call msg(MSG_INFO,'(Thompson) reading: '//trim(modelinput)//qr_acr_qs_file)
          OPEN(63,file=trim(modelinput)//qr_acr_qs_file,form="unformatted",err=1234)
!sms$serial begin
...
!.. (variable called "second") and then to the bm_s+1 moment.
        ...
        IF ( write_thompson_tables ) THEN
          write(0,*) "Writing "//trim(modelinput)//qr_acr_qs_file//" in Thompson MP init"
          OPEN(63,file=trim(modelinput)//qr_acr_qs_file,form="unformatted",err=9234)
...
!+---+
      ...
      good = 0
        INQUIRE(FILE=trim(modelinput)//freeze_h2o_file,EXIST=lexist)
!!$#ifdef MPI
!!$        call MPI_BARRIER(mpi_communicator,ierr)
!!$#endif
        IF ( lexist ) THEN
          !write(0,*) "ThompMP: read "//freeze_h2o_file//" instead of computing"
          call msg(MSG_INFO,'(Thompson) reading: '//trim(modelinput)//freeze_h2o_file)
          OPEN(63,file=trim(modelinput)//freeze_h2o_file,form="unformatted",err=1234)
!sms$serial begin
...
!$OMP END PARALLEL DO
        ...
        IF ( write_thompson_tables ) THEN
          write(0,*) "Writing "//trim(modelinput)//freeze_h2o_file//" in Thompson MP init"
          OPEN(63,file=trim(modelinput)//freeze_h2o_file,form="unformatted",err=9234)
...
!..Local variables
 ...
 2010   CONTINUE
      ...
      ENDIF
        ...
        call msg(MSG_INFO,'(Thompson) reading: '//trim(modelinput)//ccn_activate_file)
        OPEN(iunit_mp_th1,FILE=trim(modelinput)//ccn_activate_file,                      &
