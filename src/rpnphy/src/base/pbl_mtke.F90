...
contains
  ...
  subroutine moistke(en,enold,zn,zd,rif,rig,buoy,shr2,pri,qc,c1,fnn, &
    ...
    use pbl_mtke_utils, only: blcloud, tkealg, &
         BLCONST_CK, BLCONST_CU, BLCONST_CW, CLEFAE
    use pbl_diffuse, only: difuvdfj
    implicit none
...
#include "tables.cdk"
    ...
    UPDATE_TKE: if (kount == 0) then
       ...
       call tkealg(e_star,en,zn,zd,shr2,buoy,diss_term,shr_term,buoy_term,tau,n,nk)
       ...
       if (pbl_zerobc) then
          ...
          beta_sfc = 0.
          e_star(:,nk) = 0.
          call difuvdfj(en,e_star,ke,zero,zero,zero,e_sfc,beta_sfc,s,se,tau,4,1.,n,n,n,nk)
