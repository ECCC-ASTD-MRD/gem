...
!/@*
subroutine cldoppro5(taucs, omcs, gcs, taucl, omcl, gcl, &
     topthw, topthi, ecc, tcc, &
     nlow, nmid, nhigh, &
     tcclow, tccmid, tcchigh, &
     tcclowz, tccmidz, tcchighz, gz, &       
...
#include "nbsnbl.cdk"
   ...
   real, intent(out), dimension(lmx,nk,nbs) :: taucs, omcs, gcs
   real, intent(out), dimension(lmx,nk,nbl) :: taucl, omcl, gcl
   real, intent(out), dimension(lmx) :: topthw, topthi, ecc, tcc
   real, intent(out), dimension(lmx) :: nlow, nmid, nhigh
   real, intent(out), dimension(lmx) :: tcclow, tccmid, tcchigh   
   real, intent(out), dimension(lmx) :: tcclowz, tccmidz, tcchighz 
   real, intent(in) :: gz(m,nk)   
   real, intent(out), dimension(lmx) :: ctp, ctt
   real, intent(in), dimension(lmx,nk) :: liqwcin, icewcin
   real, intent(in), dimension(lmx,nk) :: liqwpin, icewpin
   real, intent(in), dimension(lmx,nk) :: cldfrac, tt, sig
   real, intent(in), dimension(lmx)    :: ps, mg, ml
   real, intent(in) :: mrk2(lmx,ens_nc2d)
...
#include "cldop.cdk"
   ...
   real, dimension(lmx) :: trmin2
   real, dimension(lmx) :: tmem2
   real, dimension(lmx) :: reifac
   real, dimension(lmx) :: rewfac
   real, dimension(lmx,nkp,nkp) :: ff
   real, dimension(lmx,nkp,nkp) :: ff2
   integer, dimension(lmx) :: ih, ih2, ih3    
   integer, dimension(lmx) :: ib, ib2, ib3    
   ...
   call init2nan(trmin, tmem, trmin2, tmem2, reifac, rewfac)
   ...
   if (etccdiag) then
      do  i=1,lmx
         tcchigh(i)  = 1. - ff2(i,1,IH2(i))
         tccmid(i)   = 1. - ff2(i,IH2(i),IB2(i))
         tcclow(i)   = 1. - ff2(i,IB2(i),nkp)
         ! to calculate 2-d cloud fraction with calipso-GOCCP height criteria
         tcchighz(i) = 1. - ff2(i,1,IH3(i))   
         tccmidz(i)  = 1. - ff2(i,IH3(i),IB3(i))
         tcclowz(i)  = 1. - ff2(i,IB3(i),nkp)
      enddo
   endif

   !----------------------------------------------------------------
   return
end subroutine cldoppro5
