...
!-------------------------------------- LICENCE END ---------------------------
   subroutine linoz_xcol( x_vmr, p_low, p_up, x_col)
...
!      cst_o3 = (1.0E-4/grav) * ( avno1 / mwt_air) / du1_o3  ! molec/cm2 -> DU
      ...
      if( x_col == 0. ) then
         x_col = cst_o3 * x_vmr * ( p_up - 0. )
       else
         x_col = x_col + cst_o3 * ( p_low - p_up ) * x_vmr 
      end if
