...
!/@*
subroutine gwspectrum5(kproma , kbdim  , klev   &
     ,s      , sh     , sexpk  &
     ,shexpk , pressg , th     &
     ,ptm1   , pum1   , pvm1   &
     ,ptte   , pvol   , pvom   &
...
#include <rmnlib_basics.hf>
   ...
   integer, intent(in) :: kproma, kbdim, klev, iheatcal, hflt
   real,    intent(in) :: non_oro_pbot        ! pressure to compute bottom level

   !  Array arguments with intent(IN):
   ! Input 1D
   real(REAL64), intent(IN) :: pressg(kproma)       ! Surface pressure (pascal)
   real, intent(IN) :: std_p_prof(klev)          ! Standard Pressure Profil (pascal)
   ! Input 2D
   real(REAL64), intent(IN) :: th(kbdim,klev)       ! half level temperature
   ! Input constants
   real, intent(IN) :: g
   real, intent(IN) :: rd
   ...
   ! - input/output 2d
   real(REAL64), intent(INOUT) :: pum1(kbdim,klev)  ! zonal wind (t-dt)
   real(REAL64), intent(INOUT) :: pvm1(kbdim,klev)  ! meridional wind (t-dt)
   real(REAL64), intent(INOUT) :: ptm1(kbdim,klev)  ! temperature (t-dt)

   !  Array arguments with intent(Out):
   ! - output 2d
   real(REAL64), intent(OUT) :: ptte(kbdim,klev)    ! tendency of temperature
   real(REAL64), intent(OUT) :: pvol(kbdim,klev)    ! tendency of meridional wind
   real(REAL64), intent(OUT) :: pvom(kbdim,klev)    ! tendency of zonal wind
   real(REAL64)              :: diffco(kbdim,klev)  ! turbulent diffusion coefficient
   ...
   real(REAL64), intent(in) :: s(kproma,klev), sh(kproma,klev), shexpk(kproma,klev), sexpk(kproma,klev)


   real(REAL64) :: dttdsf,dttdzl

   real(REAL64) :: utendgw(kproma,klev) ! zonal tend, gravity wave spectrum (m/s^2)
   real(REAL64) :: vtendgw(kproma,klev) ! merid tend, gravity wave spectrum (m/s^2)
   real(REAL64) :: ttendgw(kproma,klev) ! temperature tend, gravity wave spectrum (K/s)
   real(REAL64) ::  flux_u(kproma,klev) ! zonal momentum flux (pascals)
   real(REAL64) ::  flux_v(kproma,klev) ! meridional momentum flux (pascals)

   real(REAL64) :: uhs(kproma,klev)      ! zonal wind (m/s), input for hines param
   real(REAL64) :: vhs(kproma,klev)      ! merid wind (m/s), input for hines param
   real(REAL64) :: bvfreq(kproma,klev)   ! background brunt vassala frequency (rad/s)
   real(REAL64) :: density(kproma,klev)  ! background density (kg/m^3)
   real(REAL64) :: visc_mol(kproma,klev) ! molecular viscosity (m^2/s)
   real(REAL64) :: alt(kproma,klev)      ! background altitude (m)

   real(REAL64) :: rmswind(kproma)        ! rms gravity wave  wind, lowest level (m/s)
   real(REAL64) :: anis(kproma,nazmth)    ! anisotropy factor (sum over azimuths = 1)
   real(REAL64) :: k_alpha(kproma,nazmth) ! horizontal wavenumber of each azimuth (1/m)
   logical :: lorms(kproma)       ! .true. for rmswind /=0 at launching level

   real(REAL64) :: m_alpha(kproma,klev,nazmth) ! cutoff vertical wavenumber (1/m)
   real(REAL64) :: mmin_alpha(kproma,nazmth)   ! minumum value of m_alpha
   real(REAL64) :: sigma_t(kproma,klev)        ! total rms gw wind (m/s)
   ! gw variances from orographic sources (for coupling to a orogwd)
   real(REAL64) :: sigsqmcw(kproma,klev,nazmth), sigmatm(kproma,klev)
   ...
   call hines_extro5(kproma, klev, nazmth,             &
