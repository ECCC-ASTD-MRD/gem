 ...
 *****************************************************************************/
#include "crc24_.h"     /* include the header file generated with pycrc */
#include <stdlib.h>
#include <stdint.h>
 ...
 *****************************************************************************/
static const crc24_t crc_table[256] = {
...
crc24_t crc24_update_le(crc24_t crc, const unsigned char *data, size_t data_len, int mask)
{
    ...
    if(mask==0) return crc24_update(crc, data, data_len) ;
    
    while (data_len--) {
        tbl_idx = ((crc >> 16) ^ data[indx^mask]) & 0xff;
        crc = (crc_table[tbl_idx] ^ (crc << 8)) & 0xffffff;
