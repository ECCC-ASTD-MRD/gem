...
*
      ...
      IER = FNOM(20,'TAPE20','STD+SEQ',0)
c      IER = FNOM(6,'TAPE6','SEQ+FMT',0)
...
*
      RETURN
      END
      integer function qstint(iun)
      qstint = 0
      return
      end
      integer function qstdbg(origin,niveau)
      integer niveau
      character *(*) origin
      print *,'QSTDBG - ORIGINE=', origin, '  NIVEAU=',niveau
      qstdbg = 0
      return
      end
***FONCTION FSTCVT  HOLLERITH A CARACTERE OU L'INVERSE
*
      FUNCTION FSTCVTZZ( NOM, TYP, ETIK, GRTP, CNOM, CTYP,CETIK, CGRTP
     %, HOLACAR)
      IMPLICIT NONE
      INTEGER FSTCVTZZ
...
c      COMMON /FSTC88/ IUNTAB(0:41),DIRSIZ(0:40), NBCORR(0:40),NUTIL
c     % (0:40), NXTADR(0:40), NUMREC(0:40),PRMECR(0:40), NBECR (0:40)
c     %, NBRECR(0:40),NRECUP(0:40), NBEFF (0:40), NBEXT (0:40),MSKPRT
c     %( 6,0:40),   SEARCH( 6,0:40),MASQUE( 6,0:40),   CURSLT( 12,0:
c     %40),MODIFS(0:40),NXTSEQ(0:40),CURSEQ(0:40),DIAGP, LOGFIL,
c     % DEBUG, FASTIO, TOLRNC, MSGLVL,NPAIRE,COPYMOD, CHAINE(0:40),
c     % EFFDSC(0:40)
c      INTEGER   IUNTAB, DIRSIZ, NUTIL , NXTADR, NUMREC, MSKPRT,
c     %MASQUE, SEARCH, NBECR , NBRECR, NRECUP, NBEFF,NBEXT , NBCORR,
c     % LOGFIL, DEBUG,  CURSLT, TOLRNC,MSGLVL, NPAIRE, NXTSEQ, CURSEQ
c     %, CHAINE, EFFDSC
c      LOGICAL   PRMECR, FASTIO, MODIFS, DIAGP, COPYMOD
*
*  IUNTAB  IUNTAB(I) >= 0 EST LE NUMERO D'UNITE DU FICHIER DECRIT
*                         PAR L'ENTREE I DES TABLES DE CE BLOC.
*          IUNTAB(I) = -1 INDIQUE UNE ENTREE LIBRE.
...
c      COMMON /FSTC88/ DATE,   DEET,   NPAS,   NI,   NJ,   NK,NBITS,
c     %  DATYP,  IP1,   IP2,   IP3,   TYPVAR,NOMVAR, ETIQ14, ETIQ58,
c     % GRTYP,  IG1,   IG2,IG3,   IG4,   SWA,   LNG,   DLTF,   UBC,
c     %ETIQ56, ETIQ78
c      INTEGER   DATE,   DEET,   NPAS,   NI,   NJ,   NK,NBITS,  DATYP
c     %,  ETIQ14, NOMVAR, TYPVAR, IP1,IP2,   IP3,   GRTYP,  IG1,
c     %   IG2,   IG3,IG4,   SWA,   LNG,   DLTF,   UBC,   ETIQ58,
c     %ETIQ56, ETIQ78
c      INTEGER   TABPRM(26)
c      EQUIVALENCE   (TABPRM(1),DATE)
c      INTEGER RDSYNC, RDSYN2, SQSYNC, SQSYN2
c      COMMON /FSTSYNC/ RDSYNC, RDSYN2, SQSYNC, SQSYN2
*
**
