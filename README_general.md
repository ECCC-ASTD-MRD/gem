...
ultimately allowing the users to develop and test their GEM code.  This RDE
system was based on G-Make and it has recently shown weaknesses in areas
such as code dependencies, proper linking sequence and more importantly
transparency in controlling the compilation, linking process and makefile
target definitions. It has therefore been decided to replace RDE by a more
modern approach which is safer, hopefully more convivial and more
importantly based on public/supported software.

This new system is called GOAS and is entirely based on CMake.  The learning
curve to switch to this new system is relatively low once the user
understands the code storage structure and the location of the compiled code
and binaries. Although it is entirely possible to use GOAS with a single git
...
for each maestro suite created.
