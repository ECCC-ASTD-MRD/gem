...
cmake-debug:
	( cd build-${GEM_ARCH} && cd `/bin/pwd` && cmake -DCOMPILER_SUITE=${COMPILER_SUITE} -DCMAKE_BUILD_TYPE=Debug ${GEM_GIT_DIR} )

# Extra debug (see extra checks defined in cmake_rpn compiler presets and in CMakeLists.txt)
cmake-debug-extra:
	( cd build-${GEM_ARCH} && cd `/bin/pwd` && cmake -DCOMPILER_SUITE=${COMPILER_SUITE} -DCMAKE_BUILD_TYPE=Debug -DEXTRA_CHECKS=ON ${GEM_GIT_DIR} )
