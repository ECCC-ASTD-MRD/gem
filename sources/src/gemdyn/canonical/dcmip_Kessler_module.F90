...
SUBROUTINE KESSLER(theta, qv, qc, qr, rho, pk, dt, z, nz, precl) &
  ...
  do nt=1,rainsplit
    ...
    do k=1,nz
      ...
      qrprod = qc(k) - (qc(k)-dt0*dmax1(.001*(qc(k)-.001d0),0.0d0))/(1.d0+dt0*2.2d0*qr(k)**.875)
      qc(k) = dmax1(qc(k)-qrprod,0.0d0)
      qr(k) = dmax1(qr(k)+qrprod+sed(k),0.0d0)
      ...
      ern = dmin1(dt0*(((1.6d0+124.9d0*(r(k)*qr(k))**.2046)  &
            *(r(k)*qr(k))**.525)/(2550000d0*pc(k)            &
            /(3.8d0 *qvs)+540000d0))*(dim(qvs,qv(k))         &
            /(r(k)*qvs)),dmax1(-prod-qc(k),0.0d0),qr(k))

      ! Saturation adjustment following KW eq. 3.10
      theta(k)= theta(k) + 2500000d0/(1003.d0*pk(k))*(dmax1( prod,-qc(k))-ern)
      qv(k) = dmax1(qv(k)-max(prod,-qc(k))+ern,0.0d0)
