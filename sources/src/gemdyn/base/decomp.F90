...
#include <arch_specific.hf>
      ...
      integer, intent(in) :: F_npts, F_min, F_max, F_lni, F_npartiel, F_halo, F_start, F_npe
      ...
      integer :: istat
      integer, dimension(F_npe) :: counter, depl
!
!-------------------------------------------------------------------
!
      decomp = .false.
      ...
      if (F_checkparti_L ) then
         ...
         istat= RPN_COMM_limit_2 (0, F_npe, 1, F_npts, F_min, F_max, &
                                  counter, depl, F_relax)
         if (istat >= 0) &
         decomp = ( minval(counter) >= F_lowestsize )
         ...
         decomp = .not.(istat < 0)
         if (F_lowestsize > 0) decomp = decomp .and. (F_lni >= F_lowestsize)
