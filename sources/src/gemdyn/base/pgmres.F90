...
!
       ...
       subroutine pgmres(sol,rhs,n,niloc,njloc, nil,njl,im, eps, maxits, iout,&
                      Minx, Maxx, Miny, Maxy,F_nk,ierr)
!-----------------------------------------------------------------------
...
#include <arch_specific.hf>
       ...
       integer  Minx, Maxx, Miny, Maxy,F_nk
       integer niloc,njloc, nil,njl,n, im, maxits, iout, ierr
...
!-------------------------------------------------------------
  20   continue
         t = ddot(n, vv,1,vv,1)
       call RPN_COMM_allreduce(t,ro,1,"MPI_double_precision", &
           "MPI_sum","grid",ierr)
       ro = dsqrt(ro)
...
!     ** initialize 1-st term  of rhs of hessenberg system..
 ...
 4     i=i+1
       ...
       wk(l)=  vv(l,i)
         ...
         call pre_jacobi3D ( rhs,wk,Prec_xevec_8,niloc,njloc,&
                                   F_nk,Prec_ai_8,Prec_bi_8,Prec_ci_8 )

        call fislh_sol_matvec ( vv(1,i1), rhs,Minx, Maxx, Miny, Maxy, &
                                nil, njl, F_nk )
...
!
       call RPN_COMM_allreduce(t,t,1,"MPI_double_precision", &
           "MPI_sum","grid",ierr)
