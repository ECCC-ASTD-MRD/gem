...
!**s/r adv_BC_LAM_mask - Apply mask_o/mask_i to Tracer for Flux calculations based on Aranami et al. (2015)

      subroutine adv_BC_LAM_mask (F_adv_o,F_adv_i,F_adv,F_aminx,F_amaxx,F_aminy,F_amaxy,F_nk)
...
!$omp parallel do private (i,j,k)
      ...
      do i=F_aminx,F_amaxx
         F_adv_o(i,j,k) = adv_BC_LAM_mask_o(i,j,k) * F_adv(i,j,k)
         F_adv_i(i,j,k) = adv_BC_LAM_mask_i(i,j,k) * F_adv(i,j,k)
      end do
      end do
      end do
!$omp end parallel do
      ...
      end subroutine adv_BC_LAM_mask
