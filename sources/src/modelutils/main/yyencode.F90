...
!
      ...
      if (yinlislon.eq.0.or.yinliste(1).lt.0) then
      ...
      if (taclislon.eq.0.or.tacliste(1).lt.0) then
         write(6,'(/3x,"YIN positionnal parameters ^^ not available - ABORT"/)')
         stop
      endif
      ...
      if (yanlislon.eq.0.or.yanliste(1).lt.0) then
      ...
      DO_TICTACS: do j=1,yinlislon
         ...
         do i=1,lislon
            ...
            if(trim(var_S).ne.'!!'.and.trim(var_S).ne.'>>'.and.&
               trim(var_S).ne.'^^') then
               if (g1.eq.ip1.and.g2.eq.ip2.and.g3.eq.ip3) then
                   if (trim(var_S).eq.'UT1' .or. trim(var_S) .eq. 'URT1') then
                   p2a = g2-1
                   p3a = 2
                   else if (trim(var_S).eq.'VT1' .or. trim(var_S).eq.'VRT1') then
                   p2a = g2-2
                   p3a = 3
                   else
                   p2a = g2
