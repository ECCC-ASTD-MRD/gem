...
!
      ...
      real*8, DIMENSION(:),allocatable :: posz
...
!
      ...
      if (fnom(iun1,in_S,'RND+OLD',0).ge.0) then
      ...
      if (fnom(iun2,yin_S,'RND',0).ge.0) then
         ...
         stop
      ENDIF
      if (fnom(iun3,yan_S,'RND',0).ge.0) then
      ...
      do i=1,lislon
         ...
         if ((trim(var_S) == '^^' ) .or. (trim(var_S) == '>>' ) &
            ...
            if ( (lindex .gt. 0) .and. (grd_S(1:1) == 'U')) then
