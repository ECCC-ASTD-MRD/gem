!--------------------------------------------------------------------------
! This is free software, you can use/redistribute/modify it under the terms of
! the EC-RPN License v2 or any later version found (if not provided) at:
! - http://collaboration.cmc.ec.gc.ca/science/rpn.comm/license.html
! - EC-RPN License, 2121 TransCanada, suite 500, Dorval (Qc), CANADA, H9P 1J3
! - service.rpn@ec.gc.ca
! It is distributed WITHOUT ANY WARRANTY of FITNESS FOR ANY PARTICULAR PURPOSE.
!-------------------------------------------------------------------------- 

!/@*
subroutine test_handle_error2()
   implicit none
#include <utils4tests.hf>
   !@author  Stephane Chamberland, 2009-11
!*@/
   integer :: iErr,mype
   integer, external :: test_handle_error_cb_ok
   integer, external :: test_handle_error_cb_err
   !---------------------------------------------------------------------
   !TODO: find a way to check for not printed msg that should be printed
   mype = utils4test_toy_mpi_init('test_handle_error2')

   !- should not cause it to quit
   call handle_error_set_callBackFn(test_handle_error_cb_err)
   iErr = 0
   call handle_error(iErr,'test_handle_error',TEST_FAILED_MSG)

   !- should revert previously triggered error
   call handle_error_set_callBackFn(test_handle_error_cb_ok)
   iErr = -1
   call handle_error(iErr,'test_handle_error',TEST_FAILED_MSG)

   !- should NOT revert previously triggered error
   call handle_error_set_callBackFn(test_handle_error_cb_err)
   iErr = -1
   call handle_error(iErr,'test_handle_error',TEST_PASSED_MSG)

   call utils4test_fail('Should not get to the end of the sub')

   call utils4test_toy_mpi_end()
   !---------------------------------------------------------------------
   return
end subroutine test_handle_error2


!/@*
function test_handle_error_cb_ok(F_errorCode) result(newErrorCode)
   implicit none
   integer :: F_errorCode
   integer :: newErrorCode
   !@author  Stephane Chamberland, 2009-11
!*@/
   !---------------------------------------------------------------------
   newErrorCode = 0
   !---------------------------------------------------------------------
   return
end function test_handle_error_cb_ok

!/@*
function test_handle_error_cb_err(F_errorCode) result(newErrorCode)
   implicit none
   integer :: F_errorCode
   integer :: newErrorCode
   !@author  Stephane Chamberland, 2009-11
!*@/
   !---------------------------------------------------------------------
   newErrorCode = -1
   !---------------------------------------------------------------------
   return
end function test_handle_error_cb_err
