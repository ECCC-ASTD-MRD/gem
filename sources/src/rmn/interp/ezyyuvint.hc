...
/* interp input one grid to yygrid - no masking needed*/
  if (yyin == 0 && yyout == 1)
    ...
    icode = c_ezuvint_orig(&uuout[ni*nj],
  ...
  if (groptions.use_1subgrid == 1)
     ...
     if (groptions.valeur_1subgrid == yin_gdin)
        ...
        if (yyout == 1)
           ...
           icode = c_ezuvint_orig(&uuout[ni*nj],&vvout[ni*nj],uuin,vvin);
           }
        return icode;
        }
     if (groptions.valeur_1subgrid == yan_gdin)
        {
        icode = c_ezdefset(yin_gdout,groptions.valeur_1subgrid);
        icode = c_ezuvint_orig(uuout,vvout,&uuin[(lgdin->ni)*(lgdin->nj)],&vvin[(lgdin->ni)*(lgdin->nj)]);
        if (yyout == 1)
           {
           icode = c_ezdefset(yan_gdout,groptions.valeur_1subgrid);
           icode = c_ezuvint_orig(&uuout[ni*nj],&vvout[ni*nj],&uuin[(lgdin->ni)*(lgdin->nj)],&vvin[(lgdin->ni)*(lgdin->nj)]);
           }
        return icode;
        }
